...
export function pointsToLAS(
  ...
  if (includeColors && !actualPointFormat) actualPointFormat = 2;
  if (includeGPSTime && actualPointFormat < 1) actualPointFormat = 1;
  if (includeColors && includeGPSTime && actualPointFormat < 3) actualPointFormat = 3;
 ...
 */
function calculateScale(bounds: ReturnType<typeof calculateBounds>): [number, number, number] {
 ...
 */
function calculateOffset(bounds: ReturnType<typeof calculateBounds>): [number, number, number] {
 ...
 */
function getPointRecordLength(pointFormat: number): number {
  switch (pointFormat) {
...
function writeLASHeader(
  ...
  offset: [number, number, number],
  pointFormat: number,
  cloudMetadata: Record<string, string>,
  metadata: Record<string, string>
): void {
  let offset_bytes = 0;
  
  // File signature "LASF"
  view.setUint8(offset_bytes, 0x4C); offset_bytes++; // 'L'
  view.setUint8(offset_bytes, 0x41); offset_bytes++; // 'A'
  view.setUint8(offset_bytes, 0x53); offset_bytes++; // 'S'
  view.setUint8(offset_bytes, 0x46); offset_bytes++; // 'F'
  
  // File source ID
  view.setUint16(offset_bytes, 0, true); offset_bytes += 2;
  ...
  // Project ID GUID
  for (let i = 0; i < 16; i++) {
    view.setUint8(offset_bytes, 0); offset_bytes++;
  }
  
  // Version major and minor
  view.setUint8(offset_bytes, 1); offset_bytes++; // Version major
  view.setUint8(offset_bytes, 4); offset_bytes++; // Version minor
  
  // System identifier (32 bytes)
  const systemId = 'RF Sense Point Cloud Viewer';
  const systemIdBytes = new TextEncoder().encode(systemId.padEnd(32, '\0'));
  for (let i = 0; i < 32; i++) {
    view.setUint8(offset_bytes, systemIdBytes[i] || 0); offset_bytes++;
  }
  ...
  for (let i = 0; i < 32; i++) {
    view.setUint8(offset_bytes, softwareIdBytes[i] || 0); offset_bytes++;
  }
  
  // File creation day of year
  const now = new Date();
  const dayOfYear = Math.floor((now.getTime() - new Date(now.getFullYear(), 0, 0).getTime()) / 86400000);
  view.setUint16(offset_bytes, dayOfYear, true); offset_bytes += 2;
  
  // File creation year
  view.setUint16(offset_bytes, now.getFullYear(), true); offset_bytes += 2;
  
  // Header size
  view.setUint16(offset_bytes, 375, true); offset_bytes += 2;
  
  // Point data format ID
  view.setUint8(offset_bytes, pointFormat); offset_bytes++;
  
  // Point data record length
  view.setUint16(offset_bytes, getPointRecordLength(pointFormat), true); offset_bytes += 2;
  
  // Number of point records
  view.setUint32(offset_bytes, pointCount, true); offset_bytes += 4;
  
  // Number of points by return (5 values)
  for (let i = 0; i < 5; i++) {
    view.setUint32(offset_bytes, i === 0 ? pointCount : 0, true); offset_bytes += 4;
  }
  
  // X, Y, Z scale factors
  view.setFloat64(offset_bytes, scale[0], true); offset_bytes += 8;
  view.setFloat64(offset_bytes, scale[1], true); offset_bytes += 8;
  view.setFloat64(offset_bytes, scale[2], true); offset_bytes += 8;
  
  // X, Y, Z offsets
  view.setFloat64(offset_bytes, offset[0], true); offset_bytes += 8;
  view.setFloat64(offset_bytes, offset[1], true); offset_bytes += 8;
  view.setFloat64(offset_bytes, offset[2], true); offset_bytes += 8;
  
  // Max X, Y, Z
  view.setFloat64(offset_bytes, bounds.maxX, true); offset_bytes += 8;
  view.setFloat64(offset_bytes, bounds.maxY, true); offset_bytes += 8;
  view.setFloat64(offset_bytes, bounds.maxZ, true); offset_bytes += 8;
  
  // Min X, Y, Z
  view.setFloat64(offset_bytes, bounds.minX, true); offset_bytes += 8;
  view.setFloat64(offset_bytes, bounds.minY, true); offset_bytes += 8;
  view.setFloat64(offset_bytes, bounds.minZ, true); offset_bytes += 8;
  
  // Start of waveform data packet record (using 2x32bit instead of 64bit)
  view.setUint32(offset_bytes, 0, true); offset_bytes += 4;
  view.setUint32(offset_bytes, 0, true); offset_bytes += 4;
...
function writePointData(
  ...
  for (const point of points) {
    ...
    view.setInt32(pointOffset, lasX, true); pointOffset += 4;
    view.setInt32(pointOffset, lasY, true); pointOffset += 4;
    view.setInt32(pointOffset, lasZ, true); pointOffset += 4;
    
    // Intensity
    view.setUint16(pointOffset, Math.min(65535, Math.max(0, point.intensity || 0)), true); pointOffset += 2;
    ...
    view.setUint8(pointOffset, bitField); pointOffset += 1;
    
    // Classification
    view.setUint8(pointOffset, point.classification || 0); pointOffset += 1;
    
    // Scan angle rank
    view.setInt8(pointOffset, 0); pointOffset += 1;
    
    // User data
    view.setUint8(pointOffset, 0); pointOffset += 1;
    
    // Point source ID
    view.setUint16(pointOffset, point.pointSourceId || 0, true); pointOffset += 2;
    
    // GPS time (if format 1 or 3)
    if (pointFormat === 1 || pointFormat === 3) {
      view.setFloat64(pointOffset, point.gpsTime || Date.now() / 1000, true); pointOffset += 8;
    }
    ...
    if (pointFormat === 2 || pointFormat === 3) {
      ...
      view.setUint16(pointOffset, r, true); pointOffset += 2;
      view.setUint16(pointOffset, g, true); pointOffset += 2;
      view.setUint16(pointOffset, b, true); pointOffset += 2;
    }
  }
}
 ...
 */
export function pointsToLASJSON(
  points: Array<[number, number, number]> | Point3D[] | PointCloudData,
  options: {
  ...
  if (Array.isArray(points)) {
    ...
    if (points.metadata) {
      cloudMetadata = { ...points.metadata } as Record<string, string>;
    }
  } else {
    throw new Error('Invalid point cloud data format');
  }
 ...
 */
export async function saveLASPointCloud(
  points: Array<[number, number, number]> | Point3D[] | PointCloudData,
  filePath: string,
  options: {
    format?: 'las' | 'las_json';
