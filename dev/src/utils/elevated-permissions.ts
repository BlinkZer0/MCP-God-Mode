...
export function getElevationCommand(command: string, args: string[] = []): string {
  if (IS_WINDOWS) {
...
export async function executeInteractiveElevated(
  command: string, 
  args: string[] = [], 
  cwd?: string
): Promise<{ success: boolean; stdout?: string; stderr?: string; exitCode?: number; error?: string }> {
...
export function canElevateCommand(command: string): boolean {
