...
export function registerEmpirePowershell(server: McpServer) {
  server.registerTool("empire_powershell", {
    ...
    action, empire_host, empire_port, agent_id, module_name, listener_name, 
    listener_type, stager_type, launcher_type, target_host, file_path, 
    script_content, module_options, output_file, safe_mode, verbose 
  }) => {
    try {
...
async function startEmpire(host: string = "0.0.0.0", port: number = 1337) {
  try {
    if (PLATFORM === "win32") {
...
async function connectEmpire(host: string, port: number) {
  try {
...
async function interactAgent(agentId: string) {
  try {
    return {
      success: true,
...
async function executeModule(agentId: string, moduleName: string, options: Record<string, string> = {}, safeMode: boolean) {
  if (safeMode) {
    return {
      success: true,
...
async function uploadFile(agentId: string, filePath: string) {
  try {
    return {
      success: true,
...
async function downloadFile(agentId: string, filePath: string) {
  try {
    return {
      success: true,
...
async function takeScreenshot(agentId: string, safeMode: boolean) {
  if (safeMode) {
    return {
      success: true,
...
async function startKeylogger(agentId: string, safeMode: boolean) {
  if (safeMode) {
    return {
      success: true,
...
async function lateralMovement(agentId: string, targetHost: string, safeMode: boolean) {
  if (safeMode) {
    return {
      success: true,
...
async function establishPersistence(agentId: string, safeMode: boolean) {
  if (safeMode) {
    return {
      success: true,
...
async function privilegeEscalation(agentId: string, safeMode: boolean) {
  if (safeMode) {
    return {
      success: true,
...
async function credentialHarvest(agentId: string, safeMode: boolean) {
  if (safeMode) {
    return {
      success: true,
...
async function networkReconnaissance(agentId: string, safeMode: boolean) {
  if (safeMode) {
    return {
      success: true,
...
async function portScan(agentId: string, targetHost: string, safeMode: boolean) {
  if (safeMode) {
    return {
      success: true,
...
async function serviceEnumeration(agentId: string, targetHost: string, safeMode: boolean) {
  if (safeMode) {
    return {
      success: true,
...
async function generateStager(stagerType: string, listenerName: string) {
  try {
    return {
      success: true,
...
async function generateLauncher(launcherType: string, listenerName: string) {
  try {
    return {
      success: true,
...
async function showModuleInfo(moduleName: string) {
  try {
    return {
      success: true,
...
async function setModuleOptions(moduleName: string, options: Record<string, string>) {
  try {
    return {
      success: true,
...
async function runModule(moduleName: string, options: Record<string, string> = {}, safeMode: boolean) {
  if (safeMode) {
    return {
      success: true,
      message: "🔒 SAFE MODE: Module execution simulated. No actual module executed.",
...
async function generateReport(outputFile: string) {
  try {
    return {
      success: true,
...
async function executeCustomScript(agentId: string, scriptContent: string, safeMode: boolean) {
  if (safeMode) {
    return {
      success: true,
