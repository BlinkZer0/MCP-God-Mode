...
export function registerFridaToolkit(server: McpServer) {
  server.registerTool("frida_toolkit", {
    ...
    action, target_process, target_application, target_device, function_name, method_name, 
    class_name, module_name, memory_address, memory_size, memory_data, script_content, 
    script_file, script_type, platform, architecture, natural_language_command, safe_mode, verbose, debug
  }) => {
    try {
...
function isFridaAvailable(platform: string): boolean {
  switch (platform) {
...
async function executeWindowsFrida(action: string, params: any, options: any) {
...
async function executeLinuxFrida(action: string, params: any, options: any) {
  if (options.safe_mode === true) {
    return {
      success: true,
...
async function executeMacOSFrida(action: string, params: any, options: any) {
  if (options.safe_mode === true) {
    return {
      success: true,
...
// iOS Frida execution
async function executeIOSFrida(action: string, params: any, options: any) {
  if (options.safe_mode === true) {
    return {
      success: true,
...
async function executeAndroidFrida(action: string, params: any, options: any) {
  if (options.safe_mode === true) {
    return {
      success: true,
...
function processNaturalLanguageCommand(command: string): { action?: string; params: any } {
  ...
  if (cmd.includes('hook') && cmd.includes('function')) {
    ...
    if (funcMatch) params.function_name = funcMatch[1];
  ...
  if (cmd.includes('keychain') || cmd.includes('keystore')) {
    if (cmd.includes('ios')) return { action: 'ios_keychain_access', params };
    if (cmd.includes('android')) return { action: 'android_keystore_access', params };
    if (cmd.includes('macos')) return { action: 'macos_keychain_access', params };
  }
