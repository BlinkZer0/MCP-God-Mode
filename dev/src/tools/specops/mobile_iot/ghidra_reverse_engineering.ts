...
export function registerGhidraReverseEngineering(server: McpServer) {
  server.registerTool("ghidra_reverse_engineering", {
    ...
    action, binary_file, project_name, output_directory, analysis_depth, target_architecture, 
    target_platform, script_content, script_file, script_type, platform, architecture, 
    natural_language_command, safe_mode, verbose, debug
  }) => {
    try {
...
function isGhidraAvailable(platform: string): boolean {
  switch (platform) {
...
async function executeWindowsGhidra(action: string, params: any, options: any) {
...
async function executeLinuxGhidra(action: string, params: any, options: any) {
  if (options.safe_mode === true) {
    return {
      success: true,
...
async function executeMacOSGhidra(action: string, params: any, options: any) {
  if (options.safe_mode === true) {
    return {
      success: true,
...
async function executeIOSGhidra(action: string, params: any, options: any) {
  if (options.safe_mode === true) {
    return {
      success: true,
...
async function executeAndroidGhidra(action: string, params: any, options: any) {
  if (options.safe_mode === true) {
    return {
      success: true,
