...
const keyStore = new Map<string, { publicKey: string; privateKey: string; timestamp: number }>();
...
function generateAESKeyAndIV(password: string, salt?: Buffer) {
...
function getOrGenerateRSAKeyPair(keyId: string = "default") {
...
export function registerEncryptionTool(server: McpServer) {
  server.registerTool("encryption_tool", {
    ...
    try {
      ...
      let result = "";
      let key_info: any = { algorithm, key_size: 256 };
      if (mode) key_info.mode = mode;
      ...
      switch (action) {
        ...
        case "verify":
          if (!key) throw new Error("Public key and signature are required for verification");
