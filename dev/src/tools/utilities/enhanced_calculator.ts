...
export function registerEnhancedCalculator(server: McpServer) {
    ...
    try {
      ...
      switch (mode) {
        case "basic":
          ...
          switch (operation) {
            case "add":
              if (b === undefined) throw new Error("Second number (b) is required for addition");
              result = a + b;
              operationDescription = `Addition: ${a} + ${b}`;
              break;
            case "subtract":
              if (b === undefined) throw new Error("Second number (b) is required for subtraction");
              result = a - b;
              operationDescription = `Subtraction: ${a} - ${b}`;
              break;
            case "multiply":
              if (b === undefined) throw new Error("Second number (b) is required for multiplication");
              result = a * b;
              operationDescription = `Multiplication: ${a} × ${b}`;
              break;
            case "divide":
              if (b === undefined) throw new Error("Second number (b) is required for division");
              if (b === 0) throw new Error("Division by zero is not allowed");
              result = a / b;
              operationDescription = `Division: ${a} ÷ ${b}`;
              break;
            case "power":
              if (b === undefined) b = 2; // Default to square
              result = Math.pow(a, b);
            ...
            case "sqrt":
              ...
              break;
            case "percentage":
              if (b === undefined) b = 100; // Default to 100%
              result = (a * b) / 100;
              operationDescription = `Percentage: ${a}% of ${b}`;
              break;
            case "factorial":
              if (a < 0 || !Number.isInteger(a)) throw new Error("Factorial is only defined for non-negative integers");
              result = factorial(a);
        ...
        case "advanced":
          ...
          switch (function_name) {
            ...
            case "tanh":
              ...
              break;
            case "log":
              if (a <= 0) throw new Error("Logarithm is only defined for positive numbers");
              result = Math.log10(a);
              operationDescription = `Logarithm base 10: log(${a})`;
              break;
            case "ln":
              if (a <= 0) throw new Error("Natural logarithm is only defined for positive numbers");
              result = Math.log(a);
            ...
            case "exp":
              ...
              break;
            case "gcd":
              if (b === undefined) throw new Error("Second number (b) is required for GCD calculation");
              result = gcd(Math.abs(a), Math.abs(b));
              operationDescription = `Greatest Common Divisor: gcd(${a}, ${b})`;
              break;
            case "lcm":
              if (b === undefined) throw new Error("Second number (b) is required for LCM calculation");
...
function formatResult(result: number, format: string): string {
  switch (format) {
    case "scientific":
