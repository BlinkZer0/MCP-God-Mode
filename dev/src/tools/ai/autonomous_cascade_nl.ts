...
class CascadeNaturalLanguageProcessor {
  ...
  static parseInput(input: string): {
    ...
    let risk: "read_only" | "write_fs" | "networked" | undefined;
    for (const [riskLevel, patterns] of Object.entries(CASCADE_PATTERNS.RISK_PATTERNS)) {
      if (patterns.some((pattern: RegExp) => pattern.test(input))) {
        risk = riskLevel.toLowerCase() as "read_only" | "write_fs" | "networked";
        confidence += 0.15;
    ...
    if (isAutonomous) {
      ...
      if (!timeSeconds) timeSeconds = 1800; // 30 minutes
      if (!risk) risk = "write_fs";
      confidence += 0.1;
   ...
   */
  static generateSuggestions(parsed: ReturnType<typeof CascadeNaturalLanguageProcessor.parseInput>): string[] {
   ...
   */
  static validateConfiguration(parsed: ReturnType<typeof CascadeNaturalLanguageProcessor.parseInput>): {
    valid: boolean;
    warnings: string[];
    errors: string[];
  } {
    ...
    if (parsed.safe_mode && parsed.allowed_tools?.some(tool => 
      ["vulnerability_scanner", "exploit_framework", "malware_analysis"].includes(tool)
    )) {
 ...
 */
export function registerAutonomousCascadeNl(server: McpServer): void {
