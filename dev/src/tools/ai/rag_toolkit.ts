...
async function checkPythonDependencies(pythonCmd: string): Promise<boolean> {
  try {
    const script = `
import sys
import importlib
...
function createTempFile(prefix: string, suffix: string = '.py'): string {
...
async function executePythonScript(pythonCmd: string, script: string): Promise<any> {
...
class JavaScriptRAG {
  ...
  static searchDocuments(query: string, documents: string[], topK: number): any {
  ...
  static generateAnswer(query: string, documents: string[], contextLength: number): any {
    const results = this.searchDocuments(query, documents, 5);
    const relevantDocs = results.filter((r: any) => r.score > 0.1);
    
    let context = '';
    let currentLength = 0;
    
    for (const result of relevantDocs) {
      if (currentLength >= contextLength) break;
    ...
    const answer = `**Answer to: ${query}**\n\n` +
      ...
      `with relevance scores ranging from ${Math.min(...relevantDocs.map((r: any) => r.score)).toFixed(2)} ` +
      `to ${Math.max(...relevantDocs.map((r: any) => r.score)).toFixed(2)}.`;
  ...
  static embedText(text: string): any {
  ...
  static buildIndex(documents: string[]): any {
...
export function registerRagToolkit(server: McpServer) {
  server.registerTool("mcp_mcp-god-mode_rag_toolkit", {
    ...
    try {
      ...
      return {
        content: createContent(`RAG ${action} failed: ${error instanceof Error ? (error as Error).message : 'Unknown error'}`),
        structuredContent: {
          success: false,
          error: `RAG ${action} failed: ${error instanceof Error ? (error as Error).message : 'Unknown error'}`,
          action
        }
      };
    }
...
async function searchDocumentsJS(query: string, documents: string[], topK: number) {
...
async function queryWithContextJS(query: string, documents: string[], contextLength: number) {
...
async function embedTextJS(text: string) {
...
async function similaritySearchJS(query: string, documents: string[], topK: number, threshold: number) {
  const results = JavaScriptRAG.searchDocuments(query, documents, topK);
...
async function buildIndexJS(documents: string[]) {
...
async function retrieveContextJS(query: string, documents: string[], contextLength: number) {
  ...
  return {
    ...
    structuredContent: {
      ...
      similarities: relevantDocs.slice(0, context.length).map((r: any) => r.score),
...
async function generateAnswerJS(query: string, documents: string[], contextLength: number) {
  const result = JavaScriptRAG.generateAnswer(query, documents, contextLength);
...
async function searchDocumentsPython(query: string, documents: string[], topK: number, modelName: string, pythonCmd: string) {
  const script = `
import sys
import json
import numpy as np
...
async function queryWithContextPython(query: string, documents: string[], contextLength: number, modelName: string, answerModel: string, pythonCmd: string) {
  const script = `
import sys
import json
import numpy as np
...
async function embedTextPython(text: string, modelName: string, pythonCmd: string) {
  const script = `
import sys
import json
import numpy as np
...
async function similaritySearchPython(query: string, documents: string[], topK: number, threshold: number, modelName: string, pythonCmd: string) {
  const script = `
import sys
import json
import numpy as np
...
async function buildIndexPython(documents: string[], indexPath: string | undefined, modelName: string, pythonCmd: string) {
  const script = `
import sys
import json
import numpy as np
...
async function retrieveContextPython(query: string, documents: string[], contextLength: number, modelName: string, pythonCmd: string) {
  const script = `
import sys
import json
import numpy as np
...
async function generateAnswerPython(query: string, documents: string[], contextLength: number, answerModel: string, pythonCmd: string) {
  const script = `
import sys
import json
import numpy as np
