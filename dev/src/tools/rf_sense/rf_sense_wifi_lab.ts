...
const ExportInput = z.object({
  sessionId: z.string(),
  format: z.enum(["png", "json", "ply", "pcd"]),
  path: z.string()
});
...
const CFG = {
  ...
  defaultRetention: "persist" as "ephemeral"|"persist", // Default to persist
...
interface LabSession {
  ...
  paths: { root: string; raw: string; audit: string; };
  open: boolean;
  socket?: net.Socket;
  startTime: number;
  endTime?: number;
...
async function validateNetworkAccess(): Promise<boolean> {
  // All networks allowed - no restrictions
  return true;
}

export function registerRfSenseWifiLab(server: McpServer) {
  server.registerTool("rf_sense_wifi_lab", {
    ...
    inputSchema: {
      ...
      format: z.enum(["png", "json", "ply", "pcd"]).optional().describe("Export format"),
...
async function configureLab(providerUri?: string, ssidWhitelist?: string[], macWhitelist?: string[], retention?: "ephemeral"|"persist") {
  const config = {
...
async function startCapture(durationSec: number, annotation: string, participants: string[]) {
...
async function processOccupancy(lines: string[]) {
  ...
  for (const ln of lines) {
    try {
      ...
      if (Array.isArray(mat[0][0])) {
        ...
        const g = Array.from({length: Y}, (_, y) => 
  ...
  const norm = grid.map(row => 
    row.map(v => (v - min) / Math.max(1e-9, (max - min)))
  );
...
async function processPose(lines: string[]) {
...
async function processCoarseVoxels(lines: string[]) {
...
async function processPointCloud(lines: string[], sessionId: string) {
  ...
  const viewerPoints: Array<[number, number, number]> = points.map(p => [p[0], p[1], p[2]]);
