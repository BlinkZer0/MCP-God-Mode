...
/**
 * Unified RF Sense Tool - Consolidated RF Sensing Toolkit
 * ======================================================
 * 
 * ⚠️ CRITICAL WARNING - EXPERIMENTAL TOOL: This RF Sense tool is **experimental and untested**. 
 * It can be **extremely harmful to devices** if misused. Use only if you understand RF technology and risks. 
 * We are building the structure before reaching 100% functionality.
 * 
 * This unified tool consolidates all RF Sense capabilities into a single interface:
 * - Simulation: Synthetic data generation and testing
 * - WiFi Lab: Real Wi-Fi CSI experiments with through-wall sensing
 * - mmWave: FMCW mmWave radar with through-wall object detection
 * - Natural Language: Intuitive interface for RF sensing operations
 * - Guardrails: Safety and compliance features
 * - Localize: Point cloud localization and mapping
 */

// Import individual module functions
import { registerRfSenseSim } from "./rf_sense_sim.js";
import { registerRfSenseWifiLab } from "./rf_sense_wifi_lab.js";
import { registerRfSenseMmWave } from "./rf_sense_mmwave.js";
import { registerRfSenseNaturalLanguage } from "./rf_sense_natural_language.js";
import { registerRfSenseGuardrails } from "./rf_sense_guardrails.js";
import { registerRfSenseLocalize } from "./localize.js";
...
async function handleSimulationModule(action: string, params: Record<string, unknown>) {
  return {
    success: true,
    module: "sim",
    action: action,
...
async function handleWifiLabModule(action: string, params: Record<string, unknown>) {
  return {
    success: true,
    module: "wifi_lab", 
    action: action,
...
async function handleMmWaveModule(action: string, params: Record<string, unknown>) {
  return {
    success: true,
    module: "mmwave",
    action: action,
...
async function handleNaturalLanguageModule(action: string, params: Record<string, unknown>) {
  return {
    success: true,
...
async function handleGuardrailsModule(action: string, params: Record<string, unknown>) {
  return {
    success: true,
    module: "guardrails",
    action: action,
...
async function handleLocalizeModule(action: string, params: Record<string, unknown>) {
  return {
    success: true,
    module: "localize",
    action: action,
...
export function registerRfSenseUnified(server: McpServer) {
...
**⚠️ Safety Notice**: These tools can damage RF hardware, interfere with communications, and violate regulations if misused. Use only with proper authorization and RF engineering knowledge.`,
    ...
    try {
      ...
      const targetTool = MODULE_ROUTING[module as keyof typeof MODULE_ROUTING];
      if (!targetTool) {
      ...
      const parameterMapping = PARAMETER_MAPPING[module as keyof typeof PARAMETER_MAPPING];
      const mappedParams: Record<string, unknown> = { action };
      
      // Map known parameters
      if (parameterMapping) {
        Object.entries(parameterMapping).forEach(([unifiedKey, moduleKey]) => {
          if (params[unifiedKey as keyof typeof params] !== undefined) {
            mappedParams[moduleKey] = params[unifiedKey as keyof typeof params];
          }
        });
      }
      ...
      let result: any = { success: false, error: "Module not implemented yet" };
