...
export function registerRfSenseNaturalLanguage(server: McpServer) {
...
function parseNaturalLanguageCommand(command: string, context?: string, userIntent?: string): any {
...
function extractScenario(command: string): string {
...
function extractPipeline(command: string): string {
...
function extractFormat(command: string): string {
...
function extractCaptureMode(command: string): string {
...
function extractOutputFormat(command: string): string {
  for (const [key, value] of Object.entries(FORMAT_MAPPINGS)) {
    if (command.includes(key)) {
      return value;
    }
...
function extractVisualizeType(command: string): string {
  if (command.includes("heatmap")) return "heatmap";
  if (command.includes("point cloud") || command.includes("pointcloud")) return "pointcloud";
  if (command.includes("skeleton")) return "skeleton";
  if (command.includes("voxel")) return "voxels";
  return "heatmap";
}

function extractParticipants(command: string): string[] {
...
function extractSessionId(command: string): string {
  if (command.includes("last")) return "last";
  if (command.includes("current")) return "current";
...
function generateOutputPath(format: string, sessionId: string): string {
  const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
  const filename = `rf_sense_${sessionId}_${timestamp}.${format}`;
  return `./exports/${filename}`;
}

async function executeParsedCommand(parsedCommand: any, platform?: string): Promise<any> {
...
export function getCommandExamples(): any {
  return {
    simulation: [
      "Start a simulated RF session for 30 seconds and render occupancy",
      "Run simulation with single person scenario for 15 seconds",
