...
const CFG = {
  ...
  defaultRetention: (process.env.RF_SENSE_SIM_DEFAULT_RETENTION || "ephemeral") as "ephemeral"|"persist"
};
...
interface SimSession {
  ...
  outputFormat: string;
  resolution: string;
  paths: { root: string; data: string; audit: string; };
  consent: any;
  open: boolean;
  startTime: number;
  endTime?: number;
...
export function registerRfSenseSim(server: McpServer) {
...
async function simulateSession(durationSec: number, scenario: string, annotation: string, outputFormat: string, resolution: string) {
  const id = randomUUID();
...
async function generateSyntheticData(scenario: string, durationSec: number, resolution: string): Promise<any[]> {
  const dataPoints = Math.floor(durationSec * 10); // 10 Hz sampling
  const data: any[] = [];
  
  // Resolution settings
  const resMap = { low: 16, medium: 32, high: 64 };
  const gridSize = resMap[resolution as keyof typeof resMap] || 32;
...
function generateEmptyRoomFrame(gridSize: number, frameIndex: number): any {
  const frame = Array.from({ length: gridSize }, () => 
    Array.from({ length: gridSize }, () => Math.random() * 0.1)
  );
...
function generateSinglePersonFrame(gridSize: number, frameIndex: number): any {
  const frame = Array.from({ length: gridSize }, () => 
    Array.from({ length: gridSize }, () => Math.random() * 0.1)
  );
...
function generateMultiplePeopleFrame(gridSize: number, frameIndex: number): any {
  const frame = Array.from({ length: gridSize }, () => 
    Array.from({ length: gridSize }, () => Math.random() * 0.1)
  );
...
function generateGestureFrame(gridSize: number, frameIndex: number): any {
  const frame = Array.from({ length: gridSize }, () => 
    Array.from({ length: gridSize }, () => Math.random() * 0.1)
  );
...
function generateMotionPatternFrame(gridSize: number, frameIndex: number): any {
  const frame = Array.from({ length: gridSize }, () => 
    Array.from({ length: gridSize }, () => Math.random() * 0.1)
  );
...
async function processOccupancy(data: any[]) {
  // Aggregate frames to create occupancy heatmap
  const firstFrame = data[0]?.frame?.data;
  if (!firstFrame) return { type: "occupancy", data: [], metadata: {} };
  
  const gridSize = firstFrame.length;
  const occupancy = Array.from({ length: gridSize }, () => 
...
async function processPose(data: any[]) {
...
function findBodyParts(frame: number[][]): any[] {
  const bodyParts = [];
...
async function processCoarseVoxels(data: any[]) {
  // Convert 2D frames to 3D voxel representation
  const firstFrame = data[0]?.frame?.data;
  if (!firstFrame) return { type: "coarse_voxels", data: [], metadata: {} };
  
  const gridSize = firstFrame.length;
  const depth = Math.min(16, data.length); // 3D depth
  const voxels = Array.from({ length: depth }, () => 
...
async function processPointCloud(data: any[]) {
...
async function processGestureDetection(data: any[]) {
...
function convertToCSV(data: any): string {
  if (data.type === "point_cloud" && Array.isArray(data.data)) {
    ...
    const rows = data.data.map((point: any) => 
...
function convertToPLY(data: any): string {
  if (data.type === "point_cloud" && Array.isArray(data.data)) {
...
function convertToPCD(data: any): string {
  if (data.type === "point_cloud" && Array.isArray(data.data)) {
    const points = data.data;
...
async function visualizeSession(sessionId: string, type: string, style: string) {
  const session = sessions.get(sessionId);
  if (!session) {
    throw new Error(`Session ${sessionId} not found`);
  }
...
function generateVisualizationDescription(data: any, type: string, style: string): string {
  switch (type) {
    case "heatmap":
