...
export interface RAGQuery {
  query: string;
  system?: 'DSM-5' | 'ICD-10' | 'ICD-11' | 'all';
  category?: string;
  severity?: string;
  maxResults?: number;
...
export class PsychologyRAGSystem {
  private diagnosticDatabase: Map<string, DiagnosticReference>;
  ...
  private initializeDiagnosticDatabase(): void {
    ...
    const diagnosticData: DiagnosticReference[] = [
  ...
  async queryDiagnosticCriteria(query: RAGQuery): Promise<RAGResult> {
    const startTime = Date.now();
    const queryLower = query.query.toLowerCase();
    const results: DiagnosticReference[] = [];
  ...
  private calculateRelevanceScore(diagnostic: DiagnosticReference, query: string): number {
    let score = 0;
  ...
  private calculateConfidence(results: DiagnosticReference[], query: RAGQuery): number {
    ...
    const exactMatches = results.filter(r => 
      r.name.toLowerCase().includes(query.query.toLowerCase()) ||
  ...
  private generateSuggestions(query: RAGQuery, results: DiagnosticReference[]): string[] {
  ...
  async getDiagnosticByCode(code: string, system?: 'DSM-5' | 'ICD-10' | 'ICD-11'): Promise<DiagnosticReference | null> {
  ...
  async getDiagnosticsByCategory(category: string): Promise<DiagnosticReference[]> {
    const results: DiagnosticReference[] = [];
  ...
  async getComorbidConditions(primaryDiagnosis: string): Promise<DiagnosticReference[]> {
    const results: DiagnosticReference[] = [];
    
    for (const [id, diagnostic] of this.diagnosticDatabase) {
      if (diagnostic.comorbidity && 
          diagnostic.comorbidity.some(comorbid => 
            comorbid.toLowerCase().includes(primaryDiagnosis.toLowerCase())
          )) {
        results.push(diagnostic);
      }
    }
  ...
  getDatabaseStats(): { totalDiagnostics: number; systems: string[]; categories: string[] } {
