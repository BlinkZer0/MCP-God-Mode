...
import { z } from "zod";
import { 
  IS_ANDROID, IS_IOS, IS_MOBILE, MOBILE_CONFIG
} from "../config/environment.js";
import { 
  getMobileDeviceInfo, isMobileFeatureAvailable, getMobileProcessCommand, getMobileNetworkCommand
} from "../utils/platform.js";

interface MobileDroneCapabilities {
...
interface MobileDroneOperation {
  operationId: string;
  platform: string;
  capabilities: MobileDroneCapabilities;
  batteryLevel: number;
  networkType: string;
  operation: {
    type: string;
...
class MobileDroneManager {
  private platform: 'android' | 'ios' | 'mobile-web';
  private capabilities: MobileDroneCapabilities;
  ...
  private detectMobilePlatform(): 'android' | 'ios' | 'mobile-web' {
    if (IS_ANDROID) return 'android';
    if (IS_IOS) return 'ios';
    return 'mobile-web';
  }

  private detectMobileCapabilities(): MobileDroneCapabilities {
    const availableFeatures: string[] = [];
    const limitations: string[] = [];

    // Check available features
    if (isMobileFeatureAvailable('camera')) availableFeatures.push('camera');
    if (isMobileFeatureAvailable('location')) availableFeatures.push('location');
    if (isMobileFeatureAvailable('bluetooth')) availableFeatures.push('bluetooth');
    if (isMobileFeatureAvailable('nfc')) availableFeatures.push('nfc');
    if (isMobileFeatureAvailable('sensors')) availableFeatures.push('sensors');
    if (isMobileFeatureAvailable('notifications')) availableFeatures.push('notifications');
  ...
  private getMobileOptimizations(operationType: string): string[] {
  ...
  private getMobileCommand(operationType: string, parameters: Record<string, any>): string {
    ...
    if (parameters.intensity) args.push(`--intensity ${parameters.intensity}`);
    if (parameters.threatType) args.push(`--threat-type ${parameters.threatType}`);
  ...
  async executeMobileOperation(
    operationType: string,
    parameters: Record<string, any>
  ): Promise<MobileDroneOperation> {
    ...
    const operation: MobileDroneOperation = {
      operationId,
      platform: this.platform,
  ...
  private getMobileOperationDelay(operationType: string): number {
    ...
    if (this.batteryLevel < 30) delay *= 1.5;
    else if (this.batteryLevel < 60) delay *= 1.2;

    // Adjust for network type
    if (this.networkType === '3g') delay *= 1.3;
    else if (this.networkType === '4g') delay *= 1.1;

    return delay;
  }

  private getMobileSuccessRate(operationType: string): number {
    ...
    // Adjust for battery level
    if (this.batteryLevel < 30) rate *= 0.8;
    else if (this.batteryLevel < 60) rate *= 0.9;

    // Adjust for network type
    if (this.networkType === '3g') rate *= 0.85;
    else if (this.networkType === '4g') rate *= 0.95;

    return rate;
  }

  private calculateBatteryUsage(operationType: string, timeElapsed: number): number {
    const baseUsage = {
      'scan_surroundings': 2,
      'deploy_shield': 1.5,
      'evade_threat': 1,
    ...
    // Adjust for platform
    if (this.platform === 'ios') usage *= 0.9; // iOS is more efficient
    else if (this.platform === 'android') usage *= 1.1; // Android uses more power
    
    return Math.round(usage * 10) / 10; // Round to 1 decimal place
  }

  private calculateDataUsage(operationType: string, parameters: Record<string, any>): number {
    const baseUsage = {
      'scan_surroundings': 5,
      'deploy_shield': 2,
      'evade_threat': 1,
    ...
    if (parameters.intensity === 'high') usage *= 1.5;
    else if (parameters.intensity === 'medium') usage *= 1.2;
    
    // Adjust for target complexity
    if (parameters.target && parameters.target.includes('/24')) usage *= 1.3;
    
    return Math.round(usage * 10) / 10; // Round to 1 decimal place
  }
}

export function registerDroneMobileOptimized(server: McpServer) {
