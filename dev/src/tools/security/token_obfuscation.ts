...
export class TokenObfuscationEngine {
   ...
   */
  private createStealthHeaders(): Record<string, string> {
   ...
   */
  private randomizeRequest(req: http.IncomingMessage): void {
    if (!this.config.stealthMode.requestRandomization) return;
   ...
   */
  private hideProcessCharacteristics(): void {
    if (!this.config.stealthMode.processHiding) return;
   ...
   */
  private generateAdvancedStealthContent(content: string, targetTokens: number): string {
   ...
   */
  private validateStealthConfig(): boolean {
    try {
   ...
   */
  private async forwardRequestWithStealth(req: http.IncomingMessage, res: http.ServerResponse, targetUrl: string): Promise<void> {
    const url = new URL(targetUrl);
    const isHttps = url.protocol === 'https:';
    const httpModule = isHttps ? https : http;
   ...
   */
  private processStreamingResponseWithStealth(proxyRes: http.IncomingMessage, res: http.ServerResponse): void {
    let buffer = '';
    ...
    proxyRes.on('end', () => {
      try {
        ...
        if (json.usage) {
          ...
          const obfuscated = this.generateAdvancedStealthContent(
            JSON.stringify(json), 
            Math.floor(originalTokens * this.config.reductionFactor)
          );
