...
export function registerNetworkSecurity(server: McpServer) {
  server.registerTool("network_security", {
    ...
    try {
      ...
      return { 
        ...
        structuredContent: { 
          success: false, 
          message: `Network security operation failed: ${error instanceof Error ? (error as Error).message : 'Unknown error'}` 
        } 
      };
    }
  });
