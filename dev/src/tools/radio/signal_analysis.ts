...
export function registerSignalAnalysis(server: McpServer) {
  server.registerTool("signal_analysis", {
    ...
    try {
      ...
      let result: any = { success: true, message: "", data: {} };
      ...
      switch (action) {
        ...
        case "scan_frequencies":
          if (!frequency) throw new Error("Frequency required for scanning");
        ...
        case "decode_protocol":
          if (!frequency || !protocol) throw new Error("Frequency and protocol required for decoding");
          result = await decodeProtocol(frequency, protocol, sample_rate || 2048000, gain || 20, platform);
          break;
        
        case "spectrum_analysis":
          if (!frequency) throw new Error("Frequency required for spectrum analysis");
        ...
        case "monitor_band":
          if (!frequency) throw new Error("Frequency required for band monitoring");
          result = await monitorBand(frequency, sample_rate || 2048000, gain || 20, duration || 60, platform);
          break;
        
        case "analyze_modulation":
          if (!frequency) throw new Error("Frequency required for modulation analysis");
        ...
        case "jam_frequency":
          if (!frequency) throw new Error("Frequency required for jamming");
...
async function detectSdrDevices(platform: string) {
  try {
    const devices = [];
...
async function listSdrDevices(platform: string) {
  return await detectSdrDevices(platform);
}

async function scanFrequencies(frequency: number, sampleRate: number, gain: number, platform: string) {
  try {
...
async function captureSignal(frequency: number, sampleRate: number, gain: number, duration: number, outputFile: string, platform: string) {
  try {
...
async function decodeProtocol(frequency: number, protocol: string, sampleRate: number, gain: number, platform: string) {
  try {
    const decodedData = [];
...
async function performSpectrumAnalysis(frequency: number, sampleRate: number, gain: number, platform: string) {
  try {
    const frequencies = [];
    const powerLevels = [];
    const bandwidth = sampleRate / 2;
...
async function transmitSignal(frequency: number, audioFile: string, platform: string) {
  try {
...
async function monitorBand(frequency: number, sampleRate: number, gain: number, duration: number, platform: string) {
  try {
...
async function analyzeModulation(frequency: number, sampleRate: number, gain: number, platform: string) {
  try {
...
async function recordAudio(frequency: number, sampleRate: number, gain: number, duration: number, outputFile: string, platform: string) {
  try {
...
async function playAudio(audioFile: string, frequency: number, platform: string) {
  try {
    return {
      success: true,
...
async function jamFrequency(frequency: number, gain: number, duration: number, platform: string) {
  try {
    return {
      success: true,
...
function extractSerial(output: string): string | undefined {
