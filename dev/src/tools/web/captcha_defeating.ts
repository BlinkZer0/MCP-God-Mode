...
export function registerCaptchaDefeating(server: McpServer) {
  server.registerTool("captcha_defeating", {
    ...
    try {
      ...
      return {
        content: [{ type: "text", text: `CAPTCHA ${action} failed: ${error instanceof Error ? (error as Error).message : 'Unknown error'}` }],
        structuredContent: {
          success: false,
          error: `CAPTCHA ${action} failed: ${error instanceof Error ? (error as Error).message : 'Unknown error'}`,
          method_used: method
        }
      };
    }
...
// Helper functions
async function performCaptchaAction(
  action: string,
  url?: string,
  imagePath?: string,
  captchaType?: string,
  ...
  try {
    ...
    throw new Error(`CAPTCHA ${action} failed: ${error instanceof Error ? (error as Error).message : 'Unknown error'}`);
  }
}

async function detectCaptcha(url?: string, imagePath?: string, saveScreenshot: boolean = true) {
...
async function solveCaptcha(
  ...
  return {
    ...
    confidence,
    method: methodUsed,
    screenshotPath: `./captcha_solution_${Date.now()
      }.png`
  };
}

async function bypassCaptcha(url?: string, method: string = "auto", timeout: number = 60000) {
...
async function analyzeCaptcha(url?: string, imagePath?: string) {
  ...
  return {
    ...
    screenshotPath: `./captcha_analysis_${Date.now()
      }.png`
  };
}

async function testCaptchaSolving(url?: string, imagePath?: string, method: string = "auto") {
...
// OCR-based solving
async function solveWithOCR(imagePath?: string, captchaType?: string) {
...
// AI-based solving
async function solveWithAI(imagePath?: string, captchaType?: string) {
  // Simulate AI solving
...
// Automated solving
async function solveAutomated(url?: string, captchaType?: string) {
...
// Hybrid solving
async function solveHybrid(url?: string, imagePath?: string, captchaType?: string) {
...
async function autoDetectAndSolve(url?: string, imagePath?: string, captchaType?: string) {
