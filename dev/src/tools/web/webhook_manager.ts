...
export function registerWebhookManager(server: McpServer) {
  server.registerTool("webhook_manager", {
    description: "Advanced webhook creation, management, and monitoring toolkit",
      ...
      try {
        switch (action) {
          ...
          case "test":
            ...
            try {
              ...
              return {
                content: [{ type: "text", text: `Webhook test failed: ${error instanceof Error ? (error as Error).message : "Unknown error"}` }],
                structuredContent: {
                  success: false,
                  error: `Webhook test failed: ${error instanceof Error ? (error as Error).message : "Unknown error"}`,
                  webhook_id,
                }
              };
            }
          ...
          case "configure":
            ...
            if (name) configWebhook.name = name;
            if (url) configWebhook.url = url;
            if (method) configWebhook.method = method;
            if (headers) configWebhook.headers = { ...configWebhook.headers, ...headers };
            if (payload) configWebhook.payload = payload;
            if (port) configWebhook.port = port;
            if (timeout) configWebhook.timeout = timeout;
            if (retry_count) configWebhook.retry_count = retry_count;
            if (secret) configWebhook.secret = secret;
        ...
        return {
          content: [{ type: "text", text: `Webhook operation failed: ${error instanceof Error ? (error as Error).message : "Unknown error"}` }],
          structuredContent: {
            success: false,
...
function startWebhookServer(port: number) {
  ...
  webhookServer.on("error", (error: Error) => {
    console.error("Webhook server error:", error);
  });
}

// Helper function to trigger a webhook
async function triggerWebhook(webhook: Webhook, payload: any): Promise<any> {
  const logEntry = {
    timestamp: new Date().toISOString(),
    webhook_id: webhook.id,
    action: "outgoing",
  ...
  try {
    ...
    if (response.ok) {
      ...
      return {
        ...
        headers: Object.fromEntries(Array.from((response.headers as any).entries()) as [string, string][]),
        data: responseData,
      };
    } else {
    ...
    logEntry.error = error instanceof Error ? (error as Error).message : "Unknown error";
    ...
    if (webhook.retry_count > 0) {
      for (let i = 0; i < webhook.retry_count; i++) {
        ...
        status: retryResponse.status,
              ...
              headers: Object.fromEntries(Array.from((retryResponse.headers as any).entries()) as [string, string][]),
              retry_attempts: i + 1,
      };
          }
        } catch (retryError) {
