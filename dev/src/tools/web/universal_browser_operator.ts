...
export function registerUniversalBrowserOperator(server: McpServer) {
  ...
  server.registerTool("universal_browser_operator", {
    ...
    try {
      ...
      return {
        content: [{ type: "text", text: `Browser action failed: ${error instanceof Error ? (error as Error).message : 'Unknown error'}` }],
        structuredContent: {
          success: false,
          error: `Browser action failed: ${error instanceof Error ? (error as Error).message : 'Unknown error'}`,
          action,
          url
        }
      };
...
async function performBrowserAction(action: string, url?: string, selector?: string, text?: string, script?: string, timeout: number = 30000, headless: boolean = false) {
  ...
  try {
    ...
    switch (action) {
      ...
      case 'get_text':
        ...
        await (page as any).waitForSelector(selector, { timeout });
        result = await (page as any).textContent(selector);
        break;
        
      case 'get_html':
        if (!selector) throw new Error('Selector is required for get_html action');
        await (page as any).waitForSelector(selector, { timeout });
        result = await (page as any).innerHTML(selector);
        break;
        
      case 'evaluate':
        if (!script) throw new Error('Script is required for evaluate action');
        result = await (page as any).evaluate(script);
        break;
