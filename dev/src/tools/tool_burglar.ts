...
type BurglarAction =
  | "discover" | "preview_import" | "import_tools" | "update_tools" | "remove_tools" | "list_sources"
  | "list_local" | "enable" | "disable" | "rename" | "move" | "export" | "deprecate";

export function registerToolBurglar(server: any) {
  server.registerTool("tool_burglar", {
    ...
    const logAudit = async (event: any) => {
      if (!audit) return;
      try {
    ...
    if (params.action === "remove_tools") {
      ...
      const vendored = (plan.items || []).filter((i: any) => i.targetPath);
      for (const it of vendored) {
        try { await fs.rm(it.targetPath, { recursive: true, force: true }); } catch {}
      }
      await ensureRegisteredParity();
      await logAudit({ action: "remove_tools", count: vendored.length });
      return { 
        content: [], 
        structuredContent: { ok: true, removed: vendored.map((v: any) => v.toolName) } 
      };
    }
