...
export function registerMobileSecurityToolkit(server: McpServer) {
  server.registerTool("mobile_security_toolkit", {
    ...
    try {
      const mobileSecurityData = await performMobileSecurityAction(
        action, device_id, platform, cellular_modem, api_key, test_depth, output_format, auto_confirm
      );
      ...
      return {
        content: [{
          type: "text",
          text: `Mobile security operation failed: ${error instanceof Error ? (error as Error).message : 'Unknown error'}`
        }],
        structuredContent: {
          success: false,
          error: `Mobile security operation failed: ${error instanceof Error ? (error as Error).message : 'Unknown error'}`
        }
      };
    }
  });
...
// Helper functions
async function performMobileSecurityAction(
  action: string,
  deviceId?: string,
  platform?: string,
...
async function checkCellularAvailability(): Promise<boolean> {
  try {
...
async function performCellularTriangulation(modem: string, apiKey?: string): Promise<any> {
  try {
...
async function performDeviceAnalysis(deviceId: string, platform: string): Promise<any> {
...
async function performNetworkMonitoring(deviceId: string): Promise<any> {
...
async function performForensicAnalysis(deviceId: string): Promise<any> {
