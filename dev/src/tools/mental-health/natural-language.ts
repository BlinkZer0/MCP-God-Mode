...
export interface NaturalLanguageCommand {
  action: 'analyze' | 'find_resources' | 'security_assessment' | 'crisis_check' | 'diagnosis' | 'help';
  textSamples?: string[];
  location?: string;
  mode?: 'support' | 'security_awareness';
  searchRadius?: number;
...
export class MentalHealthNaturalLanguageProcessor {
  ...
  parseCommand(command: string): NaturalLanguageCommand {
  ...
  private extractTextSamples(command: string): string[] {
  ...
  private extractLocation(command: string): string | undefined {
  ...
  private determineAction(command: string): NaturalLanguageCommand['action'] {
  ...
  private determineMode(command: string): 'support' | 'security_awareness' {
  ...
  private extractSearchRadius(command: string): number {
  ...
  private extractBooleanFlag(command: string, keywords: string[]): boolean {
    const lowerCommand = command.toLowerCase();
...
`;
  ...
  private getPlatformInfo(): string {
    if (IS_WINDOWS) return 'Windows (Full Support)';
    if (IS_LINUX) return 'Linux (Full Support)';
    if (IS_MACOS) return 'macOS (Full Support)';
    if (IS_ANDROID) return 'Android (Mobile Optimized)';
    if (IS_IOS) return 'iOS (Mobile Optimized)';
  ...
  adaptForPlatform(params: MentalHealthToolParams): MentalHealthToolParams {
