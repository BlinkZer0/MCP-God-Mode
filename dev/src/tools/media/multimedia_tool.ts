...
async function detectMediaType(filePath: string): Promise<"audio" | "image" | "video"> {
...
async function getAudioDevices(): Promise<{ input: any[], output: any[] }> {
...
async function recordAudio(input: {
  deviceId?: string;
  deviceType?: 'microphone' | 'stereo_mix' | 'auto';
  duration?: number;
  format?: string;
  quality?: number;
  sessionName?: string;
}): Promise<{ sessionId: string; path: string; duration: number }> {
  const {
    deviceId,
    deviceType = 'auto',
    duration = 30,
  ...
  if (deviceId) {
    ...
    const stereoMixDevice = devices.input.find((d: any) => d.type === 'stereo_mix');
    const micDevice = devices.input.find((d: any) => d.type === 'microphone');
...
async function extractMetadata(filePath: string, type: "audio" | "image" | "video"): Promise<any> {
  try {
    switch (type) {
      case "image":
        const image = sharp(filePath);
...
export async function exportMedia(input: unknown) {
...
// Image Export
async function exportImage(session: Session, outputPath: string, format?: string, quality?: number, options?: any) {
...
// Audio Export
async function exportAudio(session: Session, outputPath: string, format?: string, quality?: number, options?: any) {
...
// Video Export
async function exportVideo(session: Session, outputPath: string, format?: string, quality?: number, options?: any) {
  return new Promise<void>((resolve, reject) => {
    let command = ffmpeg(session.sourcePath);
  ...
  for (const sessionId of sessionIds) {
    ...
    for (const op of operations) {
      ...
      try {
        ...
        results.push({ name: op.name, path: outputPath, success: false, error: (error as Error).message });
      }
    }
  }
...
export async function recordAudioSession(input: unknown) {
...
export async function startRecording(input: unknown) {
    ...
    try {
      ...
      switch (action) {
        case "status":
          ...
          return {
            content: [{ type: "text" as const, text: `Multimedia tool status: ${JSON.stringify(statusResult, null, 2)}` }],
            structuredContent: {
              success: true,
        ...
        case "open":
          ...
          return {
            content: [{ type: "text" as const, text: `Media opened successfully: ${openResult.sessionId}` }],
            structuredContent: {
              success: true,
        ...
        case "batch_process":
          ...
          return {
            content: [{ type: "text" as const, text: `Batch processing completed: ${batchResult.results.length} operations` }],
            structuredContent: {
              success: true,
        ...
        case "create_project":
          ...
          return {
            content: [{ type: "text" as const, text: `Project created successfully: ${projectResult.projectId}` }],
            structuredContent: {
              success: true,
        ...
        case "get_session":
          ...
          return {
            content: [{ type: "text" as const, text: `Session retrieved: ${JSON.stringify(sessionResult, null, 2)}` }],
            structuredContent: {
              success: true,
        ...
        case "delete_session":
          ...
          return {
            content: [{ type: "text" as const, text: "Session deleted successfully" }],
            structuredContent: {
              success: true,
        ...
        case "record_audio":
          ...
          return {
            content: [{ type: "text" as const, text: `Audio recorded successfully: ${recordResult.sessionId}` }],
            structuredContent: {
              success: true,
        ...
        case "get_audio_devices":
          ...
          return {
            content: [{ type: "text" as const, text: `Found ${(devicesResult as any).input.length} audio input devices` }],
            structuredContent: {
              success: true,
        ...
        case "start_recording":
          ...
          return {
            content: [{ type: "text" as const, text: startResult.message }],
            structuredContent: {
              success: true,
      ...
      return {
        content: [{ type: "text" as const, text: `Multimedia operation failed: ${error.message}` }],
        structuredContent: {
          success: false,
