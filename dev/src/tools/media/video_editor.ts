...
async function openVideo(source: string, sessionName: string, format?: string) {
  const id = newId();
...
async function editVideo(sessionId: string, op: string, params: any) {
...
async function analyzeVideo(sessionId: string) {
  const session = sessions.get(sessionId);
  if (!session) {
    throw new Error("Session not found");
  }
...
async function analyzeVideoFile(filePath: string): Promise<any> {
  try {
...
// Register the tool
export function registerVideoEditor(server: McpServer) {
  server.registerTool("video_editor", {
    ...
    try {
      switch (action) {
        ...
        case "open":
          if (!source) throw new Error("Source is required for open action");
          return await openVideo(source, sessionName || "untitled", format);
        case "record":
          if (!duration) throw new Error("Duration is required for record action");
        ...
        case "edit":
          if (!sessionId || !op) throw new Error("SessionId and op are required for edit action");
          return await editVideo(sessionId, op, params || {});
        case "export":
          if (!sessionId) throw new Error("SessionId is required for export action");
          return await exportVideo(sessionId, format, resolution, frameRate, bitrate, quality, codec, audioCodec, outputPath);
        case "delete":
          if (!sessionId) throw new Error("SessionId is required for delete action");
          return await deleteSession(sessionId);
        case "list":
          return await listSessions();
        case "analyze":
          if (!sessionId) throw new Error("SessionId is required for analyze action");
        ...
        case "open_viewer":
          if (!sessionId) throw new Error("SessionId is required for open_viewer action");
