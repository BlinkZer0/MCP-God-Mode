 ...
 * Send BadUSB script
 */
export async function sendBadusbScript(sessionId: string, script: string) {
 ...
 */
export async function sendDuckyScript(sessionId: string, duckyScript: string) {
  const session = getSession(sessionId);
 ...
 * Convert DuckyScript to BadUSB format
 */
function convertDuckyToBadusb(duckyScript: string): string {
  const lines = duckyScript.split('\n');
  const badusbCommands: string[] = [];
