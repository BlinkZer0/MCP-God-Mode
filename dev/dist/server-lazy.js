...
// ===========================================
const server = new McpServer({ name: "MCP God Mode - Lazy Loading Security & Network Analysis Platform", version: "2.0d" });
...
class LazyToolLoader {
    ...
    async loadTool(toolName) {
        ...
        try {
            ...
            const registerFunction = Object.values(toolModule).find((exported) => typeof exported === 'function' && exported.name?.startsWith('register'));
            if (registerFunction) {
...
if (_origRegisterTool) {
    server.registerTool = async (name, toolDef, handler) => {
        try {
