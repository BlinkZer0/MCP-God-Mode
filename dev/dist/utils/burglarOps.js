...
export async function planConflicts(discovered, opts = {}) {
    const items = [];
...
export async function applyWritePlan(plan, opts) {
    const writeOps = [];
    if (!plan?.items)
        return { summary: { count: 0, targets: [] } };
    ...
    if (!opts.dryRun) {
        const added = plan.items.map((i) => i.targetName);
        await patchRegistrations(added);
    }
    return { summary: { count: writeOps.length, targets: writeOps } };
}
export async function writeDocs(plan, licenseReport) {
...
export async function buildRollbackPlan(plan) {
...
export async function moveLocalTool(toolName, destDir) {
    const matches = await findFilesByPattern(`dev/src/tools/**/${toolName}.ts`);
    if (matches.length === 0)
        return { moved: 0, error: "Tool not found" };
    const src = matches[0];
...
export async function exportLocalTool(toolName, exportPath) {
    const matches = await findFilesByPattern(`dev/src/tools/**/${toolName}.ts`);
    if (matches.length === 0)
        return { exported: 0, error: "Tool not found" };
    const src = matches[0];
...
function rewriteExportNameIfNeeded(code, it) {
