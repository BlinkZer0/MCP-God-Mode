...
export function routeNaturalLanguageQuery(query) {
...
export function enhancedToolDiscovery(query, category, capability) {
    ...
    if (category) {
        filteredTools = filteredTools.filter(tool => tool.category.toLowerCase().includes(category.toLowerCase()));
    }
    if (capability) {
        filteredTools = filteredTools.filter(tool => tool.capabilities.some(cap => cap.toLowerCase().includes(capability.toLowerCase())));
    }
    return {
        tools: filteredTools,
