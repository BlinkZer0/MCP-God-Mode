import { z } from "zod";
export function registerEnhancedDataAnalysis(server) {
...
// Helper functions
function calculateStatisticalMeasures(data, outlierThreshold) {
...
function detectPatterns(data) {
    ...
    if (increasing / (data.length - 1) > 0.6)
        patterns++;
    ...
    if (decreasing / (data.length - 1) > 0.6)
        patterns++;
...
function calculateCorrelations(data, variables) {
...
function generatePredictionInsights(data) {
    const insights = [];
    if (data.length > 5) {
