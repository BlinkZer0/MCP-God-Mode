...
async function openAudio(source, sessionName, format) {
    const id = randomUUID();
...
async function recordAudio(duration, device, format = "wav", sampleRate = 44100, channels = 2, bitDepth = 16, quality = "high", enableMonitoring = true, sessionName = "recording") {
    const id = randomUUID();
    const workDir = path.join(CFG.storageDir, id);
    await fs.mkdir(workDir, { recursive: true });
...
async function editAudio(sessionId, op, params) {
...
async function exportAudio(sessionId, format, sampleRate, bitRateKbps, outputPath, quality) {
    const session = sessions.get(sessionId);
    if (!session) {
        throw new Error("Session not found");
    }
...
async function analyzeAudio(sessionId) {
    const session = sessions.get(sessionId);
    if (!session) {
        throw new Error("Session not found");
    }
...
export { getStatus, openAudio, recordAudio, editAudio, exportAudio, deleteSession, listSessions, analyzeAudio, batchProcess, openViewer, getAudioMetadata, analyzeAudioFile };
