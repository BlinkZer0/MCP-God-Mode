import { z } from "zod";
import { legalCompliance, DEFAULT_LEGAL_CONFIG } from "../../utils/legal-compliance.js";
export function registerLegalComplianceManager(server) {
        ...
        try {
            switch (action) {
                ...
                case "create_legal_hold":
                    ...
                    const legalHoldId = await legalCompliance.createLegalHold(caseName, caseDescription, createdBy, affectedData, custodian, legalBasis, caseId);
                    return {
                        content: [{
                                type: "text",
                                text: JSON.stringify({
                ...
                case "preserve_evidence":
                    ...
                    const evidenceId = await legalCompliance.preserveEvidence(sourcePath, evidenceType, metadata || {}, legalHoldIds || []);
                    return {
                        content: [{
                                type: "text",
                                text: JSON.stringify({
