...
export function registerMetasploitFramework(server) {
    ...
    }, async ({ action, target, exploit, payload, lhost, lport, rhost, rport, session_id, module, options, workspace, output_file, automation_script, custom_code, platform, architecture, natural_language_command, safe_mode, verbose }) => {
        try {
...
async function searchExploits(searchTerm) {
    try {
...
async function useExploit(exploitPath) {
    try {
...
async function setPayload(payloadPath) {
    try {
...
async function runExploit(target, safeMode) {
    if (safeMode) {
        return {
            success: true,
...
async function generatePayload(payloadPath, lhost, lport, outputFile) {
    try {
...
async function runPostModule(modulePath, sessionId) {
    try {
...
async function showInfo(modulePath) {
    try {
...
async function runAuxiliary(modulePath, target, options = {}) {
    try {
        let commands = `use ${modulePath}; `;
        if (target)
            commands += `set RHOSTS ${target}; `;
        for (const [key, value] of Object.entries(options)) {
            commands += `set ${key} ${value}; `;
        }
        commands += "run; exit";
...
async function importScan(scanFile) {
    try {
...
async function runAutomation(scriptFile) {
    try {
...
async function customExploit(code, target, safeMode) {
    if (safeMode) {
        return {
            success: true,
...
function parseExploitList(output) {
    const exploits = [];
...
function parseModuleList(output) {
    const modules = [];
    const lines = output.split('\n');
    for (const line of lines) {
        if (line.includes('/')) {
...
function extractPlatform(moduleName) {
    if (moduleName.includes('windows'))
        return 'windows';
    if (moduleName.includes('linux'))
        return 'linux';
    if (moduleName.includes('unix'))
        return 'unix';
    if (moduleName.includes('osx'))
        return 'osx';
    return 'multi';
}
function extractArch(moduleName) {
    if (moduleName.includes('x64'))
        return 'x64';
    if (moduleName.includes('x86'))
        return 'x86';
    if (moduleName.includes('arm'))
        return 'arm';
    return 'multi';
}
...
function isMetasploitAvailable(platform) {
    switch (platform) {
