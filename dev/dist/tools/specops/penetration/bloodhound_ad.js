...
export function registerBloodhoundAd(server) {
    ...
    }, async ({ action, neo4j_host, neo4j_port, neo4j_user, neo4j_password, domain, username, password, dc_ip, collection_method, query_type, cypher_query, output_format, safe_mode, verbose }) => {
        try {
...
async function startBloodhound(host = "localhost", port = 7687) {
    try {
...
async function connectNeo4j(host, port, user, password) {
    try {
        return {
            success: true,
...
async function collectData(domain, username, password, dcIp, method, safeMode) {
    if (safeMode) {
        return {
            success: true,
...
async function findAllPaths(source, target) {
    try {
        const cypherQuery = `
      MATCH (s:User {name: '${source}'}), (t:Group {name: '${target}'}),
      p = (s)-[r*1..5]->(t)
