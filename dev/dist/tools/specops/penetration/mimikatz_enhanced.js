...
export function registerMimikatzEnhanced(server) {
...
function isMimikatzAvailable(platform) {
    switch (platform) {
...
async function executeWindowsMimikatz(action, params) {
    if (params.safe_mode) {
        return {
            success: true,
...
async function executeLinuxMimikatz(action, params) {
    if (params.safe_mode) {
        return {
            success: true,
...
async function executeMacOSMimikatz(action, params) {
    if (params.safe_mode) {
        return {
            success: true,
...
async function executeIOSMimikatz(action, params) {
    if (params.safe_mode) {
        return {
            success: true,
...
async function executeAndroidMimikatz(action, params) {
    if (params.safe_mode) {
        return {
            success: true,
...
function processNaturalLanguageCommand(command) {
    ...
    if (cmd.includes('golden ticket') || cmd.includes('create golden')) {
        ...
        if (userMatch)
            params.target_user = userMatch[1];
        const domainMatch = cmd.match(/domain\s+(\w+)/);
        if (domainMatch)
            params.target_domain = domainMatch[1];
    ...
    if (cmd.includes('silver ticket') || cmd.includes('create silver')) {
        ...
        if (serviceMatch)
            params.service_name = serviceMatch[1];
    ...
    if (cmd.includes('evasion') || cmd.includes('stealth')) {
        if (cmd.includes('etw'))
            return { action: 'patch_etw', params };
        if (cmd.includes('amsi'))
            return { action: 'patch_amsi', params };
        if (cmd.includes('defender'))
            return { action: 'disable_defender', params };
