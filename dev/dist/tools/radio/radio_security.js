...
export function registerRadioSecurity(server) {
...
// Helper functions
async function performRadioAction(action, frequency, bandwidth, modulation, powerLevel, duration, audioFile, outputFile) {
    const radioData = {
        action,
        frequency,
        bandwidth,
...
async function decodeSignal(frequency, modulation) {
    const signals = {
...
async function testRadioSecurity(frequency) {
...
async function analyzeCommunication(frequency) {
...
async function monitorSpectrum(centerFreq, bandwidth, duration) {
    return await scanFrequencyRange(centerFreq, bandwidth);
}
async function detectSignal(frequency) {
