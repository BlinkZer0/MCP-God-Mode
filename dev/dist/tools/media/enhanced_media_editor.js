...
function getCrossPlatformTempDir() {
    if (isAndroid) {
...
function getCrossPlatformPath(...segments) {
    return path.join(...segments);
}
function ensureCrossPlatformDir(dirPath) {
    if (!fs.existsSync(dirPath)) {
        fs.mkdirSync(dirPath, { recursive: true });
    }
}
function getCrossPlatformFileExtension(filename) {
    return path.extname(filename).toLowerCase();
}
function isCrossPlatformImageFile(filename) {
    const ext = getCrossPlatformFileExtension(filename);
    return ['.jpg', '.jpeg', '.png', '.gif', '.bmp', '.webp', '.tiff', '.svg'].includes(ext);
}
function isCrossPlatformVideoFile(filename) {
    const ext = getCrossPlatformFileExtension(filename);
    return ['.mp4', '.avi', '.mov', '.wmv', '.flv', '.webm', '.mkv', '.m4v'].includes(ext);
}
function isCrossPlatformAudioFile(filename) {
    const ext = getCrossPlatformFileExtension(filename);
    return ['.mp3', '.wav', '.flac', '.aac', '.ogg', '.m4a', '.wma'].includes(ext);
}
// Mobile platform specific utilities
function getMobileStoragePath() {
    if (isAndroid) {
...
async function processImageCrossPlatform(inputPath, outputPath, operations) {
...
async function processAudioCrossPlatform(inputPath, outputPath, operations) {
    const capabilities = getPlatformCapabilities();
...
async function processVideoCrossPlatform(inputPath, outputPath, operations) {
    const capabilities = getPlatformCapabilities();
    if (capabilities.hasFFmpeg) {
        // Use FFmpeg for desktop platforms
...
async function processImageNative(inputPath, outputPath, operations) {
...
async function processAudioNative(inputPath, outputPath, operations) {
...
async function processVideoNative(inputPath, outputPath, operations) {
...
async function processAudioFFmpeg(inputPath, outputPath, operations) {
...
async function processVideoFFmpeg(inputPath, outputPath, operations) {
    return new Promise((resolve, reject) => {
        let command = ffmpeg(inputPath);
        for (const op of operations) {
...
function applyImageOperation(pipeline, operation) {
...
function applyAudioOperation(command, operation) {
    switch (operation.type) {
        case 'amplify':
...
function applyVideoOperation(command, operation) {
    switch (operation.type) {
        case 'resize':
...
const apiConfigurations = new Map();
...
async function generateAIImage(prompt, configId, modelId = "default") {
...
async function generateAIVideo(prompt, configId, modelId = "default") {
    const capabilities = getModelCapabilities(modelId);
    const apiConfig = configId ? getAPIConfig(configId) : null;
...
async function generateAIAudio(prompt, configId, modelId = "default") {
    const capabilities = getModelCapabilities(modelId);
    const apiConfig = configId ? getAPIConfig(configId) : null;
...
async function generateImageWithAPI(prompt, config) {
...
async function generateVideoWithAPI(prompt, config) {
    // Placeholder for actual API integration
    return {
        type: "api_video",
        prompt,
...
async function generateAudioWithAPI(prompt, config) {
    // Placeholder for actual API integration
    return {
        type: "api_audio",
        prompt,
...
async function generateAnimatedSVG(prompt) {
...
async function generateMIDIAudio(prompt) {
        ...
        try {
            ...
            switch (action) {
                ...
                case "create_session":
                    ...
                    return {
                        content: [{ type: "text", text: `GIMP 3.0 session created: ${sessionId}` }],
                        structuredContent: {
                            success: true,
                ...
                case "process_audio":
                    ...
                    return {
                        content: [{ type: "text", text: `Audacity 3.7.6 audio operation applied: ${audioResult.operationId}` }],
                        structuredContent: {
                            success: true,
                ...
                case "process_video":
                    ...
                    return {
                        content: [{ type: "text", text: `Kdenlive 25.09.0 video operation applied: ${videoResult.operationId}` }],
                        structuredContent: {
                            success: true,
                ...
                case "generate_ai_image":
                    ...
                    return {
                        content: [{ type: "text", text: `Unified AI image generated: ${aiImageResult.type}` }],
                        structuredContent: {
                            success: true,
                            message: aiImageResult.message,
                            type: aiImageResult.type,
                            prompt: aiImageResult.prompt,
                            content: aiImageResult.content || null,
                            config: aiImageResult.config || null
                        }
                    };
                case "generate_ai_video":
                    ...
                    return {
                        content: [{ type: "text", text: `Unified AI video generated: ${aiVideoResult.type}` }],
                        structuredContent: {
                            success: true,
                            message: aiVideoResult.message,
                            type: aiVideoResult.type,
                            prompt: aiVideoResult.prompt,
                            content: aiVideoResult.content || null,
                            config: aiVideoResult.config || null
                        }
                    };
                case "generate_ai_audio":
                    ...
                    return {
                        content: [{ type: "text", text: `Unified AI audio generated: ${aiAudioResult.type}` }],
                        structuredContent: {
                            success: true,
                            message: aiAudioResult.message,
                            type: aiAudioResult.type,
                            prompt: aiAudioResult.prompt,
                            tracks: aiAudioResult.tracks || null,
                            config: aiAudioResult.config || null
                        }
                    };
                case "configure_api":
                    ...
                    return {
                        content: [{ type: "text", text: apiResult.message }],
                        structuredContent: {
                ...
                case "get_api_config":
                    ...
                    return {
                        content: [{ type: "text", text: apiConfigResult ? "API configuration retrieved" : "No API configuration found" }],
                        structuredContent: {
                            success: true,
                ...
                case "detect_model_capabilities":
                    ...
                    return {
                        content: [{ type: "text", text: "Model capabilities detected" }],
                        structuredContent: {
                            success: true,
                ...
                case "generate_svg":
                    ...
                    return {
                        content: [{ type: "text", text: "SVG generated successfully" }],
                        structuredContent: {
                            success: true,
                ...
                case "generate_animated_svg":
                    ...
                    return {
                        content: [{ type: "text", text: "Animated SVG generated successfully" }],
                        structuredContent: {
                            success: true,
                ...
                case "generate_midi":
                    ...
                    return {
                        content: [{ type: "text", text: "MIDI generated successfully" }],
                        structuredContent: {
                            success: true,
