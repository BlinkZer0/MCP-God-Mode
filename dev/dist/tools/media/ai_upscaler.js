...
function getUpscalingArguments(params) {
    const { inputPath, outputPath, model, scale, customWidth, customHeight, tileSize, gpuId, ttaMode, denoise, face_enhance } = params;
...
async function fallbackUpscaling(inputPath, outputPath, params, originalSize, targetWidth, targetHeight, startTime) {
    try {
...
async function convertImageFormat(imagePath, format, compression) {
...
export function parseUpscalingCommand(command) {
    ...
    // Extract format
    if (command.includes('png'))
        params.format = 'png';
    else if (command.includes('jpg') || command.includes('jpeg'))
        params.format = 'jpg';
    else if (command.includes('webp'))
        params.format = 'webp';
