...
import { createSecuritySession, createSecurityMiddleware } from "./rf_sense_security_guard.js";
/**
...
export function registerRfSenseMmWave(server) {
    ...
    }, async ({ action, sdkPath, deviceConfig, durationSec = 300, annotation = "", participants = [], captureMode = "point_cloud", enableScanMode = false, consentGiven = false, sessionId, pipeline, format, outputPath }) => {
        try {
            assertEnabled();
...
async function configureMmWave(sdkPath, deviceConfig) {
    const config = {
...
async function startCapture(durationSec, annotation, participants, captureMode, enableScanMode = false, consentGiven = false) {
    if (!currentConfig) {
...
async function simulateMmWaveCapture(session, durationSec) {
...
function generateSimulatedMmWaveFrame(frameCount, deviceConfig) {
...
async function processPointCloud(frames, sessionId) {
...
async function processObjectTracking(frames) {
...
async function processGestureDetection(frames) {
    ...
    for (let i = 1; i < frames.length; i++) {
        ...
        if (prevFrame.points && currFrame.points) {
            ...
            const handPoints = currFrame.points.filter((p) => p.z > 1.5 && p.z < 2.5); // Hand height
            if (handPoints.length > 0) {
                const avgX = handPoints.reduce((sum, p) => sum + p.x, 0) / handPoints.length;
                const avgY = handPoints.reduce((sum, p) => sum + p.y, 0) / handPoints.length;
                gestures.push({
                    type: "hand_movement",
...
async function processFullScan(frames) {
