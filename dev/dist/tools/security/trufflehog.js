...
class TruffleHogManager {
    ...
    async findOrInstallBinary(params) {
    ...
    async installBinary(platform, architecture) {
        const urls = this.BINARY_URLS;
    ...
    async executeTruffleHog(binaryPath, args) {
    ...
    addSourceSpecificOptions(args, params) {
        // Git options
        if (params.branch) {
    ...
    async getAvailableDetectors(binaryPath) {
        try {
...
export function registerTruffleHog(server) {
