...
async function detectCaptchas(url, timeout, saveScreenshot) {
...
async function solveCaptcha(url, captchaType, solvingMethod, timeout, saveArtifacts, retryAttempts) {
    const engine = await detectBrowserEngine();
    const browser = await launchBrowser(engine, false);
    try {
        const page = await browser.newPage();
...
async function attemptSolveCaptcha(page, captchaType, method, saveArtifacts) {
    const artifacts = {};
    switch (captchaType) {
        case 'image':
...
async function solveImageCaptcha(page, method, saveArtifacts) {
...
async function solveTextCaptcha(page, method, saveArtifacts) {
...
async function solveMathCaptcha(page, method, saveArtifacts) {
...
async function solveAudioCaptcha(page, method, saveArtifacts) {
...
async function solveAutomatedCaptcha(page, captchaType, method, saveArtifacts) {
...
async function attemptBypass(url, method, customHeaders = {}, customCookies = {}, userAgent, timeout = 30000) {
    const techniques = [];
    let successRate = 0;
...
async function analyzeCaptcha(imagePath, analysisType, includeOcrPreview) {
    const analysis = {
        complexity_score: 0,
...
async function analyzeCaptchaComplexity(element, type) {
...
async function performAIVision(imagePath) {
...
async function performSpeechRecognition(audioPath) {
...
async function attemptAutomatedSolving(page, captchaType) {
...
async function attemptBypassTechniques(page, captchaType) {
