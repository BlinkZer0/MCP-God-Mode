...
async function detectForms(url, formSelector, timeout = 30000, saveScreenshot = true) {
...
async function completeForm(url, formData, formSelector, captchaHandling = 'auto', validation = true, submitForm = false, timeout = 60000, saveScreenshot = true) {
    const engine = await detectBrowserEngine();
    const browser = await launchBrowser(engine, false);
    try {
        const page = await browser.newPage();
...
async function validateFormData(formData, validationRules, strictMode = false) {
    const errors = [];
    const warnings = [];
...
async function recognizeFormPatterns(url, formSelector, timeout = 30000) {
    const engine = await detectBrowserEngine();
...
async function handleCaptcha(page, handling) {
