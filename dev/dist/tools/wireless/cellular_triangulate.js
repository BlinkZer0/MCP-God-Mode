 ...
 */
export function registerCellularTriangulate(server) {
    server.registerTool("cellular_triangulate", {
        ...
        try {
            const cellularTriangulateData = await performCellularTriangulateAction(action, modem, mode, towers, api_key, max_towers, phone_number, tower_data, gps_data, sms_method, twilio_sid, twilio_token, twilio_number, ss7_pc, ss7_gt, ss7_hlr, nl_command, auto_confirm);
            return {
                content: [{
                        type: "text",
            ...
            return {
                content: [{
                        type: "text",
                        text: `Cellular triangulation operation failed: ${error instanceof Error ? error.message : 'Unknown error'}`
                    }],
                structuredContent: {
                    success: false,
                    error: `Cellular triangulation operation failed: ${error instanceof Error ? error.message : 'Unknown error'}`
                }
            };
        }
    });
...
// Helper functions
async function performCellularTriangulateAction(action, modem, mode, towers, apiKey, maxTowers, phoneNumber, towerData, gpsData, smsMethod, twilioSid, twilioToken, twilioNumber, ss7Pc, ss7Gt, ss7Hlr, nlCommand, autoConfirm) {
    const cellularTriangulateData = {
        action,
        platform_info: {
            os: os.platform(),
...
async function checkRequestsAvailability() {
    try {
...
async function checkPpcAvailability() {
    try {
...
async function pingPhoneNumber(phoneNumber, mode, apiKey, smsMethod, twilioSid, twilioToken, twilioNumber, ss7Pc, ss7Gt, ss7Hlr, gpsData) {
...
async function performTriangulation(modem, mode, towers, apiKey, maxTowers, ss7Pc, ss7Gt, ss7Hlr, gpsData) {
...
async function scanCellularTowers(modem, maxTowers) {
...
async function queryTowerLocation(towers, apiKey) {
...
async function parseNaturalLanguageCommand(command) {
