...
// Helper functions
async function performWifiHackAction(action, targetSsid, targetBssid, securityProtocol, wirelessInterface, wordlist, attackMethod, duration, outputFile) {
    const wifiHackData = {
        action,
        target_network: {
            ssid: targetSsid,
...
async function assessNetworkSecurity(ssid, security) {
...
async function crackWEP(ssid, bssid, wifiInterface) {
...
async function crackWPA(ssid, bssid, wordlist, method, wifiInterface) {
    const startTime = Date.now();
...
async function exploitWPS(bssid, method, wifiInterface) {
    const startTime = Date.now();
...
async function enumerateIoTDevices(ssid, wifiInterface) {
    return [
        {
...
async function performPasswordAttack(ssid, wordlist, method, wifiInterface) {
    const startTime = Date.now();
...
async function scanVulnerabilities(ssid, bssid, wifiInterface) {
    const vulnerabilities = [];
    if (Math.random() > 0.6) {
...
async function disruptNetwork(ssid, bssid, wifiInterface, duration) {
    return {
