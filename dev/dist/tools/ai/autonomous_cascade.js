...
class ArtifactsManager {
    ...
    async saveArtifact(planId, round, filename, content) {
        const roundPath = await this.createRoundDirectory(planId, round);
        const artifactPath = join(roundPath, filename);
        await fs.writeFile(artifactPath, content);
        return artifactPath;
    }
    async loadLastCheckpoint(planId) {
        try {
            const runPath = join(this.basePath, planId);
...
class SafetyManager {
    ...
    static validateAction(action, spec) {
...
class PlanExecutor {
    ...
    async executeAction(action, spec) {
...
class AutonomousCascadeController {
    ...
    async generatePlan(spec, context, planId) {
    ...
    summarizeResults(results) {
        return results.map(r => `${r.action_type}${r.tool_name ? ` (${r.tool_name})` : ''}: ${r.success ? 'SUCCESS' : 'FAILED'}`).join(', ');
    }
    generateNextSuggestions(plan, results) {
        const suggestions = [
 ...
 */
export function registerAutonomousCascade(server) {
