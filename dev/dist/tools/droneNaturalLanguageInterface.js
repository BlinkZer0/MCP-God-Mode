...
class DroneNaturalLanguageProcessor {
    ...
    static parseCommand(command, context) {
        ...
        const naturalLanguageResponse = this.generateNaturalLanguageResponse(action, parameters, platform, safetyChecks);
        return {
            parsedCommand: {
                action: action.type,
                parameters,
    ...
    static getPlatformOptimizations(platform, mobileCapabilities) {
        const optimizations = [];
        if (IS_MOBILE) {
    ...
    static performSafetyChecks(action, parameters) {
    ...
    static generateSuggestedActions(action, parameters, platform) {
        const suggestions = [];
    ...
    static generateNaturalLanguageResponse(action, parameters, platform, safetyChecks) {
        ...
        if (safetyChecks.legalWarnings.length > 0) {
            response += `\n**⚠️ Legal Warnings:**\n`;
            safetyChecks.legalWarnings.forEach((warning) => {
                response += `• ${warning}\n`;
            });
        }
        return response;
...
export function registerDroneNaturalLanguageInterface(server) {
