...
async function listDirectoryRecursive(dirPath: string, pattern?: string): Promise<string[]> {
  ...
  for (const item of items) {
    ...
    if (item.isDirectory()) {
      results.push(...await listDirectoryRecursive(fullPath, pattern));
    } else {
...
async function compressFile(source: string, destination: string, compressionType: string): Promise<void> {
  const stats = await fs.stat(source);
  
  if (stats.isDirectory()) {
    // For directories, create a zip file
    const { stdout } = await execAsync(`zip -r "${destination}" "${source}"`);
  } else {
    // For single files, use appropriate compression
    switch (compressionType) {
      case "zip":
        await execAsync(`zip "${destination}" "${source}"`);
        break;
      case "gzip":
...
async function main() {
  ...
  console.log("🚀 **MCP GOD MODE - REFACTORED SERVER STARTED**");
  console.log("📊 Core Tools Available: health, system_info, fs_list, fs_read_text, fs_write_text, fs_search, file_ops, proc_run, proc_run_elevated");
  console.log("");
  console.log("🔧 **CORE FUNCTIONALITY LOADED**");
  console.log("📁 File System Tools: Advanced file operations, search, and management");
  console.log("⚙️ Process Tools: Process execution with elevated privileges");
  console.log("🖥️ System Tools: System information and health monitoring");
  console.log("");
