    ...
    try {
        if (IS_WINDOWS) {
            ...
            try {
                ...
                if (stdout.includes('utf-8'))
                    return 'utf8';
                if (stdout.includes('utf-16'))
                    return 'utf16le';
                if (stdout.includes('iso-8859'))
                    return 'latin1';
                if (stdout.includes('windows-1252'))
                    return 'cp1252';
            }
            catch {
...
async function findFiles(searchPath, options, startTime) {
    const files = [];
...
async function performGrepSearch(pattern, searchPath, options) {
...
export function registerAdvancedGrep(server) {
    server.registerTool("advanced_grep", {
        ...
        try {
            const { pattern, path: searchPath, caseInsensitive = false, wholeWord = false, regex = false, contextBefore = 0, contextAfter = 0, maxFileSize = 10485760, // 10MB
            encoding = "auto", outputFormat = "text", colorOutput = true, showLineNumbers = true, showFilename = true, recursive = true, includePattern = [], excludePattern = [], maxDepth = 0, limitResults = 0, binaryFiles = "skip", followSymlinks = false, performanceMode = "balanced" } = params;
