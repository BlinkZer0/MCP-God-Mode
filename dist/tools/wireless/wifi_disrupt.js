 ...
 */
export function registerWifiDisrupt(server) {
    server.registerTool("wifi_disrupt", {
        ...
        try {
            const wifiDisruptData = await performWifiDisruptAction(action, wifiInterface, mode, target_bssid, channel, duration, power, nl_command, auto_confirm);
            return {
                content: [{
                        type: "text",
            ...
            return {
                content: [{
                        type: "text",
                        text: `Wi-Fi disruption operation failed: ${error instanceof Error ? error.message : 'Unknown error'}`
                    }],
                structuredContent: {
                    success: false,
                    error: `Wi-Fi disruption operation failed: ${error instanceof Error ? error.message : 'Unknown error'}`
                }
            };
        }
    });
...
// Helper functions
async function performWifiDisruptAction(action, wifiInterface, mode, targetBssid, channel, duration, power, nlCommand, autoConfirm) {
    const wifiDisruptData = {
        action,
        platform_info: {
            os: os.platform(),
...
async function checkScapyAvailability() {
    try {
...
async function performDeauthFlood(wifiInterface, targetBssid, channel, duration) {
    try {
...
async function performMalformedSpam(wifiInterface, channel, duration) {
    try {
        // Use Python script for actual packet injection
        const pythonScript = path.join(__dirname, 'wifi_disrupt.py');
        const args = [
...
async function performAirtimeOccupation(wifiInterface, channel, duration) {
    try {
        // Use Python script for actual packet injection
        const pythonScript = path.join(__dirname, 'wifi_disrupt.py');
        const args = [
