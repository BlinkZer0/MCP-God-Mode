...
// Helper functions
async function performWirelessScan(iface, execAsync) {
    try {
        let command = "";
        if (PLATFORM === "win32") {
...
function parseWirelessScanOutput(output, platform) {
    const networks = [];
...
async function getConnectedNetwork(execAsync) {
    try {
        let command = "";
        if (PLATFORM === "win32") {
...
function parseConnectedNetwork(output, platform) {
    if (platform === "win32") {
        const lines = output.split('\n');
        for (const line of lines) {
...
async function getSignalStrength(iface, execAsync) {
    try {
        let command = "";
        if (PLATFORM === "win32") {
            command = "netsh wlan show interfaces";
...
function parseSignalStrength(output, platform) {
    if (platform === "win32") {
        const lines = output.split('\n');
        for (const line of lines) {
