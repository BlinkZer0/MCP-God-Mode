...
import { upscaleImage, batchUpscaleImages, parseUpscalingCommand, recommendModel, getModelsByCategory, validateUpscalingParams, UPSCALING_MODELS } from "./ai_upscaler.js";
// Set FFmpeg path
...
async function handleImageUpscaling(params) {
    const { source, path: outputPath, upscaleModel = 'realesrgan-x4plus', upscaleScale, upscaleWidth, upscaleHeight, tileSize = 512, gpuId, cpuThreads, ttaMode = false, denoise = true, faceEnhance = false, preserveMetadata = true, format = 'png', compression = 90, naturalLanguageCommand, 
    // New comprehensive Upscayl features
    modelCategory, seamlessTextures = false, alphaChannel = true, colorProfile = 'auto', batchMode = false, outputNaming = 'suffix', customSuffix = '_upscaled', memoryLimit, enableMemoryOptimization = true, postProcessing = {
        sharpen: false,
        sharpenAmount: 0.5,
        colorCorrection: false,
        contrastEnhancement: false
    }, autoRecommendModel = false } = params;
    if (!source) {
...
async function handleBatchUpscaling(params) {
    const { inputPaths, outputDir, upscaleModel = 'realesrgan-x4plus', upscaleScale, upscaleWidth, upscaleHeight, tileSize = 512, gpuId, cpuThreads, ttaMode = false, denoise = true, faceEnhance = false, preserveMetadata = true, format = 'png', compression = 90, naturalLanguageCommand, 
    // New comprehensive Upscayl features
    modelCategory, seamlessTextures = false, alphaChannel = true, colorProfile = 'auto', batchMode = true, // Always true for batch operations
    outputNaming = 'suffix', customSuffix = '_upscaled', memoryLimit, enableMemoryOptimization = true, postProcessing = {
        sharpen: false,
        sharpenAmount: 0.5,
        colorCorrection: false,
        contrastEnhancement: false
        ...
        try {
            ...
            switch (action) {
                ...
                case "get_upscale_models":
                    ...
                    return {
                        content: [{ type: "text", text: `Found ${models.length} upscaling models${category ? ` in category '${category}'` : ''}` }],
                        structuredContent: {
                            success: true,
                ...
                case "recommend_model":
                    ...
                    const modelInfo = UPSCALING_MODELS[recommendedModel];
                    return {
                        content: [{ type: "text", text: `Recommended model: ${modelInfo.name} (${recommendedModel})` }],
                        structuredContent: {
                            success: true,
