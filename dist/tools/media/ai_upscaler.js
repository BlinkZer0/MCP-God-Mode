...
function getUpscalerBinaryPath() {
        ...
        try {
            ...
            if (platform === 'win32') {
                ...
                try {
                    ...
                    hasVulkan = gpuLines.some(line => line.toLowerCase().includes('nvidia') ||
...
function getUpscalingArguments(params) {
    const { inputPath, outputPath, model, scale, customWidth, customHeight, tileSize, gpuId, cpuThreads, ttaMode, denoise, face_enhance, seamlessTextures, alphaChannel, memoryLimit, enableMemoryOptimization } = params;
...
export async function upscaleImage(inputPath, outputPath, params, onProgress) {
    ...
    try {
        ...
        if (!targetWidth && !targetHeight && !targetScale) {
            ...
            const modelInfo = UPSCALING_MODELS[params.model];
...
async function executeUpscaling(binaryPath, args, onProgress) {
...
async function fallbackUpscaling(inputPath, outputPath, params, originalSize, targetWidth, targetHeight, startTime, onProgress) {
    try {
...
async function applyPostProcessing(imagePath, postProcessing) {
    ...
    if (postProcessing.sharpen) {
        pipeline = pipeline.sharpen(postProcessing.sharpenAmount, // sigma
        1, // flat
        2 // jagged
        );
    }
...
async function convertImageFormat(imagePath, format, compression, colorProfile = 'auto') {
...
async function copyImageMetadata(sourcePath, destPath) {
    try {
...
export async function batchUpscaleImages(inputPaths, outputDir, params, onProgress) {
    const startTime = Date.now();
    const results = [];
    // Ensure output directory exists
    if (!fs.existsSync(outputDir)) {
    ...
    for (let i = 0; i < inputPaths.length; i++) {
        ...
        try {
            const result = await upscaleImage(inputPath, outputPath, params, (itemProgress, message) => {
...
export function parseUpscalingCommand(command) {
    const params = {
        postProcessing: {
            sharpen: false,
            sharpenAmount: 0.5,
    ...
    if (lowerCommand.includes('png'))
        params.format = 'png';
    else if (lowerCommand.includes('jpg') || lowerCommand.includes('jpeg'))
        params.format = 'jpg';
    else if (lowerCommand.includes('webp'))
        params.format = 'webp';
    else if (lowerCommand.includes('tiff') || lowerCommand.includes('tif'))
        params.format = 'tiff';
    else if (lowerCommand.includes('bmp'))
        params.format = 'bmp';
    ...
    if (lowerCommand.includes('srgb'))
        params.colorProfile = 'srgb';
    else if (lowerCommand.includes('adobe rgb'))
        params.colorProfile = 'adobe-rgb';
    else if (lowerCommand.includes('prophoto'))
        params.colorProfile = 'prophoto-rgb';
...
export function recommendModel(imagePath, userPreferences) {
    return new Promise(async (resolve) => {
        try {
...
export function getModelsByCategory(category) {
    ...
    return Object.values(UPSCALING_MODELS).filter(model => model.category === category);
}
// Validate upscaling parameters
export function validateUpscalingParams(params) {
    const errors = [];
