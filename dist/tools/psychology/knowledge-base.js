...
export class PsychologyKnowledgeBase {
    ...
    calculateRelevanceScore(resource, query) {
        let score = 0;
    ...
    calculateConfidence(results, query) {
        ...
        const exactMatches = results.filter(r => r.title.toLowerCase().includes(query.query.toLowerCase()) ||
    ...
    generateEthicalWarning(query, results) {
        const darkPsychologyResults = results.filter(r => r.type === 'dark_psychology' ||
            r.type === 'manipulation' ||
