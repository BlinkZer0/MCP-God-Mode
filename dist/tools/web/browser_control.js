...
export function registerBrowserControl(server) {
...
async function navigateToUrl(browser, url, headless) {
    try {
...
async function typeText(browser, selector, text) {
    try {
        const instances = Array.from(browserInstances.entries());
        if (instances.length === 0) {
            throw new Error('No browser instance available');
...
function getLatestBrowserInstance() {
    const instances = Array.from(browserInstances.entries());
    if (instances.length === 0)
        return null;
