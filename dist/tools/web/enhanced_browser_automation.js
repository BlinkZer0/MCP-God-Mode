...
export function registerEnhancedBrowserAutomation(server) {
    ...
    }, async ({ action, browser = "auto", headless = false, viewport, url, selector, xpath, text, script, script_type = "execute", form_data, wait_time = 5000, timeout = 30000, file_path, output_file, user_agent, geolocation, blocked_resources, workflow_steps, session_id }) => {
        try {
            ...
            switch (action) {
                ...
                case "click":
                    if (!selector && !xpath && !text)
                        throw new Error("Selector, xpath, or text is required for click action");
                    result = await clickElement(targetBrowser, selector, xpath, text, session_id);
                    break;
                case "type":
                case "fill":
                    if (!text)
                        throw new Error("Text is required for type/fill action");
                    result = await typeText(targetBrowser, text, selector, xpath, session_id);
                    break;
                case "hover":
                    if (!selector && !xpath)
                        throw new Error("Selector or xpath is required for hover action");
                ...
                case "get_text":
                    if (!selector && !xpath)
                        throw new Error("Selector or xpath is required for extract action");
                ...
                case "inject_script":
                    if (!script)
                        throw new Error("Script is required for script execution");
                    result = await executeScript(targetBrowser, script, script_type, session_id);
                    break;
                case "form_fill":
                    if (!form_data)
                        throw new Error("Form data is required for form fill action");
                ...
                case "wait_for_element":
                    if (!selector && !xpath)
                        throw new Error("Selector or xpath is required for wait_for_element");
                    result = await waitForElement(targetBrowser, selector, xpath, timeout, session_id);
                    break;
                case "wait_for_text":
                    if (!text)
                        throw new Error("Text is required for wait_for_text");
                    result = await waitForText(targetBrowser, text, timeout, session_id);
                    break;
                case "upload_file":
                    if (!file_path || !selector)
                        throw new Error("File path and selector are required for upload");
                ...
                case "set_geolocation":
                    if (!geolocation)
                        throw new Error("Geolocation coordinates are required");
                    result = await setGeolocation(targetBrowser, geolocation, session_id);
                    break;
                case "block_resources":
                    if (!blocked_resources)
                        throw new Error("Blocked resources list is required");
...
async function launchBrowser(browser, headless, viewport, userAgent, sessionId) {
...
async function navigateToUrl(browser, url, headless, sessionId) {
    try {
...
async function clickElement(browser, selector, xpath, text, sessionId) {
    try {
...
async function typeText(browser, text, selector, xpath, sessionId) {
    try {
        const instance = getBrowserInstance(sessionId);
        if (!instance)
            throw new Error('No browser instance available');
...
async function takeScreenshot(browser, outputFile, sessionId) {
    try {
...
async function executeScript(browser, script, scriptType, sessionId) {
    try {
        const instance = getBrowserInstance(sessionId);
        if (!instance)
            throw new Error('No browser instance available');
...
async function extractContent(browser, action, selector, xpath, sessionId) {
    try {
        const instance = getBrowserInstance(sessionId);
        if (!instance)
            throw new Error('No browser instance available');
...
async function fillForm(browser, formData, sessionId) {
    try {
        const instance = getBrowserInstance(sessionId);
        if (!instance)
            throw new Error('No browser instance available');
...
async function closeBrowser(browser, sessionId) {
    try {
        if (sessionId) {
            const instance = browserInstances.get(sessionId);
            if (instance) {
                if (instance.type === 'playwright') {
                    const playwrightBrowser = instance.browser;
                    await playwrightBrowser.close();
                }
                else {
                    const puppeteerBrowser = instance.browser;
...
// Helper functions
function getBrowserInstance(sessionId) {
    if (sessionId) {
...
async function launchSystemBrowser(browser) {
    try {
        let command = "";
        switch (browser.toLowerCase()) {
            case "chrome":
...
async function navigateSystemBrowser(browser, url) {
    try {
        let command = "";
        switch (browser.toLowerCase()) {
            case "chrome":
...
async function browserNavigation(browser, action, sessionId) {
    const instance = getBrowserInstance(sessionId);
    if (!instance)
        throw new Error('No browser instance available');
    const page = instance.page;
...
async function hoverElement(browser, selector, xpath, sessionId) {
    const instance = getBrowserInstance(sessionId);
    if (!instance)
        throw new Error('No browser instance available');
    const page = instance.page;
...
async function scrollPage(browser, sessionId) {
    const instance = getBrowserInstance(sessionId);
    if (!instance)
        throw new Error('No browser instance available');
    const page = instance.page;
...
async function extractHTML(browser, selector, xpath, sessionId) {
    const instance = getBrowserInstance(sessionId);
    if (!instance)
        throw new Error('No browser instance available');
    const page = instance.page;
...
async function submitForm(browser, selector, sessionId) {
    const instance = getBrowserInstance(sessionId);
    if (!instance)
        throw new Error('No browser instance available');
    const page = instance.page;
...
async function waitForElement(browser, selector, xpath, timeout = 30000, sessionId) {
    const instance = getBrowserInstance(sessionId);
    if (!instance)
        throw new Error('No browser instance available');
    const page = instance.page;
...
async function waitForText(browser, text, timeout = 30000, sessionId) {
    const instance = getBrowserInstance(sessionId);
    if (!instance)
        throw new Error('No browser instance available');
    const page = instance.page;
    await page.waitForFunction((searchText) => document.body.innerText.includes(searchText), text, { timeout });
    return `Text found: ${text}`;
}
async function uploadFile(browser, filePath, selector, sessionId) {
    const instance = getBrowserInstance(sessionId);
    if (!instance)
        throw new Error('No browser instance available');
    const page = instance.page;
...
async function setViewport(browser, viewport, sessionId) {
    const instance = getBrowserInstance(sessionId);
    if (!instance)
        throw new Error('No browser instance available');
    const page = instance.page;
...
async function setGeolocation(browser, geolocation, sessionId) {
    const instance = getBrowserInstance(sessionId);
    if (!instance)
        throw new Error('No browser instance available');
...
async function blockResources(browser, blockedResources, sessionId) {
    const instance = getBrowserInstance(sessionId);
    if (!instance)
        throw new Error('No browser instance available');
    const page = instance.page;
...
async function automateWorkflow(browser, workflowSteps, sessionId) {
    const results = [];
