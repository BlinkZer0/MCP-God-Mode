import { z } from "zod";
export function registerHackGPTNaturalLanguage(server) {
...
function generateSuggestions(action) {
    const suggestions = {
        reconnaissance: [
...
function calculateConfidence(parsedCommand) {
    ...
    if (parsedCommand.target)
        confidence += 0.2;
    if (parsedCommand.tool_integration)
        confidence += 0.15;
    if (parsedCommand.attack_vector)
        confidence += 0.1;
    if (Object.keys(parsedCommand.additional_params || {}).length > 0)
        confidence += 0.05;
    // Decrease confidence for ambiguous commands
    if (parsedCommand.action === "reconnaissance" && !parsedCommand.tool_integration)
        confidence -= 0.1;
