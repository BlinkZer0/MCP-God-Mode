import { z } from "zod";
export function registerMetadataExtractor(server) {
...
function checkMetadataStripping(input) {
...
function getMetadataSurvivalChance(input) {
    const platform = detectPlatform(input);
    switch (platform) {
        case 'Reddit':
        case 'Imgur':
...
function getPlatformWarning(input) {
    const platform = detectPlatform(input);
