 ...
 */
import { z } from "zod";
import { assertEnabled, assertUsbEnabled, assertBleEnabled, newSession, endSession, listSessions, getConfig } from './session.js';
import { usbTransport, findFlipperUsbDevices } from './transport/usbSerial.js';
import { bleTransport, scanForFlipperDevices, isBleAvailable } from './transport/ble.js';
 ...
 */
export function registerFlipperZeroTool(server) {
                ...
                case "list_devices": {
                    ...
                    if (args.include_bridge && config.bridgeUrl) {
                        try {
                            ...
                            const bridgeDevices = await new Promise((resolve) => {
                                const list = [];
                                const ws = new WS(config.bridgeUrl);
                                const timer = setTimeout(() => { try {
                                    ws.close();
                                }
                                catch { } ; resolve(list); }, 3500);
                                ws.onopen = () => {
                                    try {
 ...
 */
export function getFlipperZeroToolName() {
    return 'flipper_zero';
}
