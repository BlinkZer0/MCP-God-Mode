import { ALL_DIAGNOSTIC_CRITERIA, PSYCHOLOGICAL_MARKERS } from './diagnostic-data.js';
...
export class MentalHealthAnalyzer {
    ...
    async analyzeTextSamples(textSamples) {
    ...
    extractPsychologicalProfile(text) {
        const markers = {};
    ...
    calculateMarkerScore(text, marker) {
        let totalScore = 0;
        let keywordCount = 0;
    ...
    determineEmotionalTone(markers) {
        const toneScores = {
    ...
    matchDiagnosticCriteria(profile) {
        const matches = [];
        for (const criteria of this.diagnosticCriteria) {
            const matchScore = this.calculateDiagnosticMatch(profile, criteria);
            if (matchScore > 0.3) {
                const matchedIndicators = criteria.indicators.filter(indicator => profile.markers[indicator] > 0.2);
    ...
    calculateDiagnosticMatch(profile, criteria) {
        let totalScore = 0;
    ...
    calculateDiagnosticConfidence(matchScore, matchedCount, totalCount) {
        const coverageRatio = matchedCount / totalCount;
        return Math.min(1, matchScore * coverageRatio);
    }
    determineSeverity(matchScore, markers) {
        if (matchScore > 0.8)
            return 'Severe';
        if (matchScore > 0.6)
            return 'Moderate';
    ...
    identifySpecifiers(profile, criteria) {
        const specifiers = [];
    ...
    matchesSpecifier(profile, specifier) {
        const specifierMap = {
    ...
    assessCrisisLevel(profile, diagnoses) {
    ...
    identifyRiskFactors(profile, diagnoses) {
        const riskFactors = [];
    ...
    generateRecommendations(profile, diagnoses, crisisLevel) {
    ...
    getDiagnosisSpecificRecommendations(diagnosis) {
        const recommendations = [];
    ...
    calculateConfidence(profile, diagnoses) {
