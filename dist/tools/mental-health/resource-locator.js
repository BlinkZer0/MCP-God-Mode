...
export class ResourceLocator {
    ...
    async findLocalResources(location, diagnoses, searchRadius = 25) {
    ...
    findResourcesInRadius(location, specialties, radius) {
        ...
        if (specialties.length > 0) {
            resources = resources.filter(r => specialties.some(specialty => r.specialties.some(s => s.toLowerCase().includes(specialty.toLowerCase()))));
        }
    ...
    findEmergencyResources(location, radius) {
    ...
    findSupportGroups(location, specialties, radius) {
        ...
        // Filter by specialties
        if (specialties.length > 0) {
            groups = groups.filter(g => specialties.some(specialty => g.specialties.some(s => s.toLowerCase().includes(specialty.toLowerCase()))));
        }
