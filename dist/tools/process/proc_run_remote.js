...
export function registerProcRunRemote(server) {
    ...
    }, async ({ target_host, target_port, protocol, command, args = [], working_dir, username, password, key_file, elevated = false, timeout = 30, platform = 'auto', capture_output = true, interactive = false, passwordless = false, auto_auth = true, custom_protocol_config = {} }) => {
        try {
            const startTime = Date.now();
            const connectionStartTime = Date.now();
            // Validate and prepare connection with passwordless authentication
            const connection = await preparePasswordlessConnection({
                protocol,
                host: target_host,
            ...
            return {
                ...
                structuredContent: {
                    success: false,
                    message: `Remote process execution failed: ${error.message}`,
                    target_host,
                    protocol
                }
            };
...
async function preparePasswordlessConnection(options) {
    const { protocol, host, port, username, password, keyFile, timeout, elevated, passwordless, auto_auth, custom_config } = options;
...
async function detectAvailableAuthMethods(protocol, host, port) {
    const authMethods = {};
    try {
        switch (protocol) {
            case 'ssh':
...
async function detectPlatform(connection) {
    try {
        ...
        if (connection.port === 5985 || connection.port === 5986)
            return 'windows';
        if (connection.port === 5555)
            return 'android';
...
async function executeRemoteCommand(connection, command, args, working_dir, options = { elevated: false, capture_output: true, interactive: false, custom_config: {} }) {
...
async function executeSSHCommand(connection, command, args, options) {
    try {
...
async function executeWinRMCommand(connection, command, args, options) {
    try {
        const fullCommand = args.length > 0 ? `${command} ${args.join(' ')}` : command;
...
async function executeADBCommand(connection, command, args, options) {
    try {
...
async function executeIOSDeployCommand(connection, command, args, options) {
    try {
...
async function executeTelnetCommand(connection, command, args, options) {
    try {
        const fullCommand = args.length > 0 ? `${command} ${args.join(' ')}` : command;
        const elevatedCmd = options.elevated ? `sudo ${fullCommand}` : fullCommand;
...
async function executeCustomProtocolCommand(connection, command, args, options) {
    try {
