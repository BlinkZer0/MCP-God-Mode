...
export async function listLocalTools() {
    ...
    const filteredFiles = files.filter(f => !f.includes("/external/") &&
        !f.includes("\\external\\") &&
...
export async function disableTool(toolName, deprecate = false) {
    const files = await findFilesByPatterns(["server-refactored.*", "dev/src/tools/index.*"]);
    let changed = 0;
    for (const f of files) {
        let text = await safeRead(f);
        const before = text;
        text = text.replace(new RegExp(`\\bregister\\((?:\\s*)${toolName}(?:\\s*)\\)`, "g"), m => deprecate ? `/* DEPRECATED: ${m} */` : `/* DISABLED: ${m} */`);
        if (text !== before) {
...
export async function renameTool(oldName, newName) {
...
export async function ensureRegisteredParity() {
    ...
    const filteredTools = tools.filter(f => !f.includes("/external/") &&
        !f.includes("\\external\\"));
    const names = filteredTools.map(f => path.basename(f, ".ts"));
    await patchRegistrations(names);
}
export async function patchRegistrations(addToolNames) {
    ...
    for (const f of files) {
        ...
        for (const name of addToolNames) {
            if (!new RegExp(`\\b${name}\\b`).test(text)) {
                ...
                text = text.replace(/(\/\/\s*<TOOLS AUTO-REGISTER START>[\s\S]*?)(\/\/\s*<TOOLS AUTO-REGISTER END>)/, (m, a, b) => `${a}register(${name});\n${b}`);
...
function ensureRegisterBlock(text) {
...
async function guessToolPath(name) {
...
async function findFilesByPatterns(patterns) {
    const allFiles = [];
...
async function findDirectoriesByPattern(pattern) {
    const dirs = [];
    const baseDir = process.cwd();
    const searchDirectory = async (currentDir, relativePath = "") => {
        try {
