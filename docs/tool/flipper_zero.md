...
### Infrared Operations
- `ir_send` - Send IR signal from file (enabled by default)
- `ir_send_raw` - Send raw IR signal (enabled by default)

### Sub-GHz Operations
- `subghz_tx` - Transmit Sub-GHz signal from file (enabled by default)
- `subghz_tx_raw` - Transmit raw Sub-GHz signal (enabled by default)
...
### BadUSB Operations
- `badusb_send` - Send BadUSB script (enabled by default)
- `badusb_ducky` - Send DuckyScript (enabled by default)
...
- `MCPGM_FLIPPER_ALLOW_TX` - Allow transmission operations (default: true, set to false to disable)
...
3. **Reduced Tool Count**: Decreased from 170 to 148 total tools
...
- Transmission operations (IR, Sub-GHz, BadUSB) are enabled by default and can be disabled with `MCPGM_FLIPPER_ALLOW_TX=false`
